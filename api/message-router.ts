...
export const messageRouter = createRouter({
  ...
  exportConversation: supportQuery()
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const header = [
        "FLEXHAVENS — Conversation Transcript",
        line,
        ...
        `Created   : ${new Date(conv.createdAt).toLocaleString("en-GB")}`,
        line,
        "",
      ];
      const body = msgs.map((m) => {
        const who = m.senderType === "user" ? (inv?.name ?? "Customer") : m.senderType === "system" ? "FlexHavens (System)" : `${m.senderName} (Admin)`;
        const text = m.deleted === "yes" ? "[message deleted]" : m.body;
        const att = m.attachmentName ? `\n[Attachment: ${m.attachmentName}]` : "";
        return `[${new Date(m.createdAt).toLocaleString("en-GB")}] ${who}:\n${text}${att}\n`;
      });

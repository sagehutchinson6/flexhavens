...
export const investAdminRouter = createRouter({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (input.decision === "approved") {
          ...
          if (isFirstApproved && investor.referredById) {
            const bonus = (amount * ReferralBonus.percentOfFirstDeposit) / 100;
          ...
          depositDoc = { investorId: deposit.investorId, name: "", email: "", amount, reference, method: deposit.method, depositId: deposit.id };
          await tx
            .update(investmentTransactions)
            .set({
              status: "failed",
      ...
      if (input.decision === "approved" && depDoc) {
        void generatePdfDocument({
          ...
          note: "Your deposit has been approved and the funds credited to your FlexHavens wallet.",
        });
      }
      return result;
    }),
  ...
  reviewWithdrawal: investAdminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      let wdInvestor: { id: number; amount: number; reference: string } | null = null;
      const result = await db.transaction(async (tx) => {
        ...
        wdInvestor = { id: withdrawal.investorId, amount, reference };
      ...
      const wdInv = wdInvestor as { id: number; amount: number; reference: string } | null;
      if (wdInv) {

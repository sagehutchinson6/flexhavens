...
/** Standard PDF fonts lack the ₦ glyph — use the ISO code for clean output. */
export function pdfMoney(value: number | string): string {
  const n = typeof value === "string" ? Number(value) : value;
  if (!isFinite(n)) return "NGN 0.00";
...
async function buildPdf(input: DocGenInput, docRef: string, createdAt: Date): Promise<string> {
  ...
  y += 8;
  const dateStr = createdAt.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" });
  ...
  doc.setTextColor(...GRAY);
  doc.text(
    "This document was generated electronically by FlexHavens Real Estate Development Ltd. and is valid without a physical signature.",
    W / 2,
    288,
    { align: "center" }
  );
  doc.text(`${docRef}  ·  ${Company.email}  ·  Abuja, FCT, Nigeria`, W / 2, 293, { align: "center" });

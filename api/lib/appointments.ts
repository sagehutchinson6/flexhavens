...
/** Combine a YYYY-MM-DD date and HH:mm time into a server-local Date (WAT). */
...
export function fmtWhen(d: Date): string {
  return `${d.toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })} at ${d.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })} (WAT)`;
}
          ...
          <div style="font-family:Georgia,serif;background:#faf8f5;padding:32px">
            <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #eee">
              <div style="background:#1e3a5f;padding:20px 28px">
                <span style="color:#ffffff;font-size:20px;font-weight:bold;letter-spacing:1px">FLEXHAVENS</span>
                <span style="color:#c8956c;font-size:11px;display:block;letter-spacing:2px;text-transform:uppercase">Real Estate Development Ltd</span>
              </div>
              <div style="padding:28px">
                ...
                <p style="color:#aaa;font-size:11px"><a href="${baseUrl}" style="color:#c8956c">${baseUrl}</a></p>
              </div>
            </div>
          </div>`,
...
export async function generateAppointmentConfirmationPdf(appt: Appointment): Promise<string> {
  ...
  doc.setFontSize(8);
  doc.text("Abuja, FCT, Nigeria", 14, 29.5);
  ...
  const dateStr = when.toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long", year: "numeric" });
  ...
  doc.setFontSize(7.5);
  doc.text(
    "This confirmation was generated electronically by FlexHavens Real Estate Development Ltd. and is valid without a physical signature.",
    W / 2,
    288,
    { align: "center" },
  );
  doc.text(`${appt.appointmentRef}  ·  ${Company.email}  ·  Abuja, FCT, Nigeria`, W / 2, 293, { align: "center" });

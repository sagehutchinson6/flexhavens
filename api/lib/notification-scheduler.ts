...
async function sweep() {
    ...
    if (m <= now) {
      ...
      if (await claim(`maturity1:${inv.id}`, inv.investorId)) {
        await notifyUser(inv.investorId, {
          ...
          message: `Your investment in ${inv.projectName} matures tomorrow (${m.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}).`,
          link: "/invest/dashboard?tab=portfolio",
          relatedRef: `INV-${inv.id}`,
          emailDetails: [
            { label: "Project", value: inv.projectName },
            { label: "Amount Invested", value: fmtMoney(inv.amount) },
            { label: "Maturity Date", value: m.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" }) },
          ],
        });
      }
    } else if (m <= in7d) {
      if (await claim(`maturity7:${inv.id}`, inv.investorId)) {
        await notifyUser(inv.investorId, {
          ...
          message: `Your investment in ${inv.projectName} matures in 7 days (${m.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}).`,
          link: "/invest/dashboard?tab=portfolio",
          relatedRef: `INV-${inv.id}`,
          emailDetails: [
            { label: "Project", value: inv.projectName },
  ...
  for (const mg of activeMortgages) {
    ...
    const dueStr = due.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" });
    if (due > now && due <= in3d) {
  ...
  if (now.getDate() === 1 && now.getHours() < 1) {
    ...
    for (const pref of opted.slice(0, 200)) {
      ...
      await generatePdfDocument({
        ...
        note: "This statement summarizes your FlexHavens Invest account activity for the period shown.",

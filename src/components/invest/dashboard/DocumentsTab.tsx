  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        action={<FolderOpen className="w-5 h-5 text-[#c8956c]" />}
      >
        {/* Category chips */}
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition ${
              category === null ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            }`}
          >
            All
          </button>
            ...
            return (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition flex items-center gap-1.5 ${
                  category === c.key ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}
              >
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-5">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by name, property, reference, investment or mortgage…"
              className="w-full border border-gray-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30"
            />
          </div>
          <select
            value={status}
            ...
            {docs.map((d) => {
              ...
              return (
                <div key={d.id} className="py-3.5 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#1e3a5f]/5 flex items-center justify-center shrink-0">
                    <Icon className="w-5 h-5 text-[#1e3a5f]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold text-[#1e3a5f] text-sm truncate">{d.name}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="font-semibold text-[#1e3a5f] text-sm truncate pr-4">{previewUrl.name}</p>

...
const reasons = [
  ...
  "I found another investment platform",
...
export default function DeleteAccountDialog({ onClose }: { onClose: () => void }) {
  ...
  const checkRows = checks
    ? [
        { ok: checks.activeInvestments === 0, label: "No active investments" },
        { ok: checks.pendingInvestments === 0, label: "No pending investments" },
        ...
        { ok: checks.pendingLiquidations === 0, label: "No pending liquidation requests" },
                  ...
                  {!eligible && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5">
                      ...
                      <ul className="text-xs text-amber-800 space-y-1 list-disc pl-4">
                        <li>Liquidate all active investments and wait for liquidation approval.</li>
          ...
          {step === 2 && (
            <>
                ...
                {reasons.map((r) => (
                  <label
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg border text-sm cursor-pointer transition ${
                      reason === r ? "border-[#1e3a5f] bg-[#1e3a5f]/5 font-semibold text-[#1e3a5f]" : "border-gray-200 text-gray-600"
                    }`}
                  >
                    <input type="radio" name="delete-reason" checked={reason === r} onChange={() => setReason(r)} className="accent-[#1e3a5f]" />
              ...
              <textarea
                ...
                className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30 focus:border-[#1e3a5f] resize-none mb-5"
              />
              <div className="flex gap-3">

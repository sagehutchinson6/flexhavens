      ...
      {steps.map((s, i) => (
        <div key={i} className="flex items-center flex-1 last:flex-none">
          <div className="flex flex-col items-center">
            <div className={`w-3 h-3 rounded-full border-2 ${s.done ? "bg-[#c8956c] border-[#c8956c]" : "bg-white border-gray-300"}`} />
            <span className={`text-[10px] mt-1 whitespace-nowrap ${s.done ? "text-[#1e3a5f] font-semibold" : "text-gray-400"}`}>
              {s.label}
            </span>
            {s.at && (
          ...
          {i < steps.length - 1 && <div className={`h-0.5 flex-1 mx-1 rounded ${steps[i + 1].done ? "bg-[#c8956c]" : "bg-gray-200"}`} />}
        </div>
      ))}
    </div>
  );
  ...
  const renderCard = (a: Appt, isUpcoming: boolean) => (
    <div key={a.id} className="border border-gray-200 rounded-xl p-4 sm:p-5 hover:shadow-md transition">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h4 className="font-bold text-[#1e3a5f]">{appointmentTypeLabel(a.type)}</h4>
        ...
        <div className="text-right text-sm">
          <p className="font-semibold text-[#1e3a5f] flex items-center gap-1.5 justify-end">
            <CalendarDays className="w-4 h-4 text-[#c8956c]" />
      ...
      <div className="mt-3 grid sm:grid-cols-2 gap-2 text-sm text-gray-600">
        {a.propertyName && (
          <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#c8956c] shrink-0" /> {a.propertyName}</p>
        )}
        {a.location && (
          <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#c8956c] shrink-0" /> {a.location}</p>
        )}
        {a.meetingLink && ["confirmed", "rescheduled"].includes(a.status) && (
          <a href={a.meetingLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[#1e3a5f] underline">
            <Video className="w-4 h-4 text-[#c8956c] shrink-0" /> Join meeting
          </a>
        )}
        {a.assignedAdminName && (
          <p className="flex items-center gap-2"><UserRound className="w-4 h-4 text-[#c8956c] shrink-0" /> Consultant: {a.assignedAdminName}</p>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        action={
          <Button onClick={() => setBookingOpen(true)} className="bg-[#1e3a5f] hover:bg-[#2d5a87]">
      ...
      {rescheduleTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <h3 className="font-serif text-xl font-bold text-[#1e3a5f] mb-1">Reschedule Appointment</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30"
                />
              </div>
              <div>
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30"

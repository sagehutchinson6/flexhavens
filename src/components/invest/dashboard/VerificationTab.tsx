...
function DocUploadSlot({
  ...
  return (
    <div className={`border-2 border-dashed rounded-xl p-4 transition ${file ? "border-green-300 bg-green-50/50" : "border-gray-200 hover:border-[#c8956c]/50"}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        {file ? (
          ...
          <label className="shrink-0 cursor-pointer px-3 py-1.5 rounded-lg bg-[#1e3a5f] text-white text-xs font-medium hover:bg-[#2d5a87] transition flex items-center gap-1.5">
...
export default function VerificationTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Current level */}
      <SectionCard title="Verification Status" subtitle="Your verification level determines your deposit and investment limits">
          ...
          <div className="flex-1">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-5">
              <div className="rounded-xl bg-[#faf8f5] p-4">
                ...
                <p className="text-lg font-bold text-[#1e3a5f] font-serif mt-1">{formatCurrency(data.limits.maxDeposit)}</p>
              </div>
              <div className="rounded-xl bg-[#faf8f5] p-4">
                <p className="text-xs text-gray-500">Maximum Investment (per transaction)</p>
                <p className="text-lg font-bold text-[#1e3a5f] font-serif mt-1">{formatCurrency(data.limits.maxInvestment)}</p>
              </div>
            </div>
            <div className="mt-5">
            ...
            {KYC_TIERS.map((t) => (
              <div
                ...
                className={`rounded-xl border p-3 ${
                  t.key === data.tier ? "border-[#c8956c] bg-[#c8956c]/5" : "border-gray-200"
                }`}
              >
                <div className="flex items-center justify-between">
                  <VerificationBadge tier={t.key} size="sm" />
                  {t.key === data.tier && <span className="text-[10px] font-bold text-[#c8956c] uppercase">Current</span>}
                </div>
                  ...
                  <div className="space-y-1.5">
                    {data.openDocuments.map((d) => (
                      <p key={d.id} className="text-sm text-gray-600 flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#c8956c]" />
                        {kycDocTypeLabel(d.docType)} — {d.name}
                        <a href={d.dataUrl} download={d.name} className="text-[#1e3a5f] hover:underline ml-1">
      ...
      {!open && nextTierInfo && (
        <SectionCard
          ...
          action={<ArrowUpCircle className="w-5 h-5 text-[#c8956c]" />}
        >
          <div className="space-y-4">
            <div className="rounded-xl bg-[#faf8f5] p-4">
              ...
              <>
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    maxLength={2000}
                    placeholder="Describe the origin of the funds you invest with FlexHavens (e.g. employment income, business revenue, property sales)…"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30 resize-y"
                  />
                </div>
                <DocUploadSlot
                  docType="additional"
      ...
      {/* History */}
      <SectionCard title="Verification History" subtitle="Every verification event on your account" action={<History className="w-5 h-5 text-[#c8956c]" />}>
        {data.history.length === 0 ? (
          <p className="text-sm text-gray-400 py-4">No verification activity yet.</p>
        ) : (
          <div className="space-y-3">
            {data.history.map((h) => (
              <div key={h.id} className="flex items-start gap-3 border-l-2 border-[#c8956c]/30 pl-3">
                <div className="flex-1">
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold capitalize text-[#1e3a5f]">{h.action.replace(/_/g, " ")}</span>

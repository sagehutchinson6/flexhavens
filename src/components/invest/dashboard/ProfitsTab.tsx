...
export default function ProfitsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title="Monthly Profit Payments"
        subtitle="ROI credited to your wallet every month"
      >
        {profitsQuery.isLoading ? (
          ...
          <EmptyState
            ...
            text="Once your investment is approved and active, profits are credited to your wallet every month."
          />
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wider text-gray-400 border-b">
                    ...
                    <th className="pb-3 pr-4 font-semibold">ROI Rate</th>
                  ...
                  {profits.map((p: any) => (
                    <tr key={p.id} className="hover:bg-[#faf8f5] transition">
                      <td className="py-3.5 pr-4">
                        <p className="font-semibold text-[#1e3a5f]">{p.projectName}</p>
                        <p className="text-xs text-gray-400">Investment #{p.investmentId}</p>
                      </td>
                      <td className="py-3.5 pr-4 font-medium">
                        Month {p.monthNumber}
                      </td>
                      <td className="py-3.5 pr-4 text-[#c8956c] font-semibold">
            ...
            <div className="md:hidden space-y-3">
              {profits.map((p: any) => (
                <div
                  key={p.id}
                  className="bg-[#faf8f5] rounded-xl p-4 border border-gray-100 flex items-center justify-between gap-3"
                >
                  <div className="min-w-0">
                    <p className="font-semibold text-[#1e3a5f] truncate">{p.projectName}</p>

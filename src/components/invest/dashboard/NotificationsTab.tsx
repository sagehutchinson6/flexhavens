    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-lg font-bold text-[#1e3a5f] font-serif">Notification Preferences</h3>
      ...
      <SectionCard
        ...
        action={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setPrefsOpen(true)} className="border-[#1e3a5f] text-[#1e3a5f]">
              <Settings2 className="w-4 h-4 mr-2" />
              Preferences
            </Button>
            <Button variant="outline" size="sm" onClick={() => markAll.mutate()} disabled={markAll.isPending} className="border-[#1e3a5f] text-[#1e3a5f]">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          {FILTERS.map((f) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold transition ${
                filter === f.key ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}
            >
              {f.label}
            </button>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 w-52"
            />
          </div>
          <select
            value={category}
            ...
            className="px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 text-gray-600"
          >
            ...
            {items.map((n) => {
              ...
              return (
                <div
                  key={n.id}
                  onClick={open}
                  className={`w-full text-left flex gap-4 p-4 rounded-xl border transition cursor-pointer hover:border-[#c8956c]/50 ${
                    n.isRead === "no" ? "bg-[#faf8f5] border-[#c8956c]/30" : "bg-white border-gray-100"
                  }`}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className={`text-sm font-semibold ${n.isRead === "no" ? "text-[#1e3a5f]" : "text-gray-600"}`}>{n.title}</p>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      ...
                      {cta && n.link && (
                        <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#c8956c] hover:text-[#b07d52] transition">
                          {cta.label}
                    ...
                    <div className="flex flex-col gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                      {filter === "archived" ? (
                        <button
                          ...
                          title="Unarchive"
                          className="p-2 hover:bg-gray-100 rounded-lg transition text-gray-400 hover:text-[#1e3a5f]"
                        >

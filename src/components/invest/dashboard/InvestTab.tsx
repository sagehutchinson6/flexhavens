...
export default function InvestTab({
  ...
  const invest = trpc.investor.invest.useMutation({
    onSuccess: () => {
      toast.success("Investment submitted for review. It will be activated once approved by our team.");
      setAmountStr("");
      onInvested();
      setTab("portfolio");
    },
    ...
    <div className="grid lg:grid-cols-5 gap-6">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-5 rounded-xl border-2 transition ${
                    selected
                      ? "border-[#1e3a5f] bg-[#1e3a5f]/[0.03] shadow-md"
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-[#1e3a5f] flex items-center gap-2">
                          {plan.name}
                          {plan.featured === "yes" && (
                            <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-[#c8956c]/10 text-[#b07d52] px-2 py-0.5 rounded-full">
                    ...
                    <p className={`text-2xl font-bold font-serif ${selected ? "text-[#c8956c]" : "text-[#1e3a5f]"}`}>
                      {plan.targetReturn}%
                    </p>
                  </div>
                  {selected && (
            ...
            <div className="space-y-3">
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border-2 transition ${
                  projectId === undefined ? "border-[#1e3a5f] bg-[#1e3a5f]/[0.03]" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                <p className="font-semibold text-[#1e3a5f] text-sm">Diversified Portfolio (recommended)</p>
              ...
              {projects.map((project: any) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl border-2 transition flex items-center gap-4 ${
                    projectId === project.id ? "border-[#1e3a5f] bg-[#1e3a5f]/[0.03]" : "border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <img
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-[#1e3a5f] text-sm truncate">{project.name}</p>
      ...
      <div className="lg:col-span-2">
        <div className="bg-[#1e3a5f] rounded-2xl p-6 text-white sticky top-24">
          <h3 className="text-xl font-bold font-serif mb-6">Investment Summary</h3>

          <div className="bg-white/10 rounded-xl p-4 mb-5 flex items-center justify-between">
            <span className="text-sm text-gray-300 flex items-center gap-2">
              <Wallet className="w-4 h-4 text-[#c8956c]" />
              Wallet Balance
            </span>
          ...
          <div className="relative mt-2 mb-1">
            ...
            <Input
              ...
              className="pl-8 h-12 bg-white text-[#1e3a5f] font-bold text-lg"
            />
          </div>
          ...
          <div className="space-y-3 text-sm border-t border-white/10 pt-4 mb-6">
            ...
            <div className="flex justify-between text-base pt-2 border-t border-white/10">
              <span className="text-gray-200">Value at Maturity</span>
              <span className="font-bold font-serif text-[#c8956c]">
          ...
          <Button
            ...
            className="w-full h-12 bg-gradient-to-r from-[#c8956c] to-[#b07d52] hover:shadow-xl transition text-base font-semibold"
          >

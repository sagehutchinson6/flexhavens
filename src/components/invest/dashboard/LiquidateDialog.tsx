...
export default function LiquidateDialog({ investmentId, onClose, onSuccess }: LiquidateDialogProps) {
  ...
  const requestMutation = trpc.investor.requestLiquidation.useMutation({
    onSuccess: () => {
      toast.success("Liquidation request submitted for review");
      onSuccess();
      onClose();
    },
          ...
          {estimateQuery.isLoading ? (
            ...
            <>
              <div className="bg-[#faf8f5] rounded-xl p-4 mb-4">
                <p className="text-xs text-gray-400 uppercase tracking-wider">Investment</p>
                <p className="font-bold text-[#1e3a5f]">{est.projectName}</p>
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#1e3a5f]">{formatCurrency(est.principal)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#1e3a5f]">{formatCurrency(est.monthlyProfit)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Accrued This Month</span>
                  <span className="font-semibold text-[#1e3a5f]">{formatCurrency(est.accruedProfit)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Early Liquidation Penalty ({est.penaltyPercent}%)</span>
                ...
                <div className="border-t pt-2.5 flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-[#c8956c]">{formatCurrency(est.estimatedValue)}</span>
                </div>
              </div>

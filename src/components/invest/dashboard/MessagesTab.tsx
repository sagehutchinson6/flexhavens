...
function fmtTime(d: Date | string): string {
  ...
  if (dt.toDateString() === today.toDateString()) return dt.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
...
export default function MessagesTab() {
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-[#1e3a5f] border border-gray-200 rounded-full px-3 py-1.5 hover:border-[#c8956c] transition"
            >
      ...
      >
              ...
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] sm:max-w-[75%] rounded-2xl px-4 py-3 text-sm shadow-sm ${
                    system
                      ? "bg-[#faf8f5] border border-[#c8956c]/30 text-gray-700"
                      : mine
                ...
                >
                  <div className={`flex items-center gap-2 mb-1 ${mine ? "justify-end" : ""}`}>
                    {system && <ShieldCheck className="w-3.5 h-3.5 text-[#c8956c]" />}
                    <span className={`text-[11px] font-semibold ${mine ? "text-white/80" : "text-[#1e3a5f]"}`}>
                      {system ? "FlexHavens" : m.senderName}
                    </span>
                  ...
                  {m.attachmentName && m.attachmentUrl && m.deleted !== "yes" && (
                    <button
                      ...
                      className={`mt-2 flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition ${
                        mine ? "bg-white/10 hover:bg-white/20 text-white" : "bg-[#1e3a5f]/5 hover:bg-[#1e3a5f]/10 text-[#1e3a5f]"
                      }`}
                    >
        ...
        {active?.status === "open" ? (
          <div className="mt-4 border-t border-gray-100 pt-4">
            {replyFile && (
              <div className="mb-2 flex items-center gap-2 text-xs bg-[#1e3a5f]/5 text-[#1e3a5f] rounded-lg px-3 py-2 w-fit">
            ...
            <div className="flex items-end gap-2">
              <label className="w-10 h-10 shrink-0 rounded-full border border-gray-200 flex items-center justify-center text-gray-400 hover:text-[#1e3a5f] hover:border-[#c8956c] cursor-pointer transition" title="Attach file">
              ...
              />
              <Button onClick={sendReply} disabled={replyMutation.isPending} className="bg-[#1e3a5f] hover:bg-[#2d5a87] rounded-full w-10 h-10 p-0 shrink-0">
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="Messages"
        subtitle="Secure communication with the FlexHavens team — like internet banking messaging"
        action={
          <Button onClick={() => setComposeOpen(true)} className="bg-[#1e3a5f] hover:bg-[#2d5a87]">
      ...
      >
        {listQuery.isLoading ? (
          ...
          <EmptyState
            ...
            text="Message our team about properties, investments, mortgages, payments or anything else."
            action={
              <Button onClick={() => setComposeOpen(true)} className="bg-[#1e3a5f] hover:bg-[#2d5a87]">
            ...
            {convs.map((c) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-3 py-4 hover:bg-[#faf8f5] rounded-lg transition flex items-center gap-3"
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${c.systemGenerated === "yes" ? "bg-[#c8956c]/10" : "bg-[#1e3a5f]/5"}`}>
                    <MessageSquare className={`w-4 h-4 ${c.systemGenerated === "yes" ? "text-[#c8956c]" : "text-[#1e3a5f]"}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`text-sm truncate ${c.unread ? "font-bold text-[#1e3a5f]" : "font-medium text-gray-700"}`}>{c.subject}</p>
                      {c.unread && <span className="w-2 h-2 rounded-full bg-[#c8956c] shrink-0" />}
                    </div>
      ...
      {composeOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-lg w-full shadow-2xl p-6">
            ...
            <h3 className="font-serif text-xl font-bold text-[#1e3a5f] mb-4">New Message</h3>
            <div className="space-y-3">
              ...
              <div>
                {composeFile ? (
                  <div className="flex items-center gap-2 text-xs bg-[#1e3a5f]/5 text-[#1e3a5f] rounded-lg px-3 py-2 w-fit">
                    <Paperclip className="w-3.5 h-3.5" />

...
export default function MortgagesTab({
  ...
  const pay = trpc.mortgage.payMortgage.useMutation({
    onSuccess: (res) => {
      toast.success(
        res.completed
          ? "Payment received — congratulations, your mortgage is fully paid! 🎉"
  ...
  const downloadStatement = async (m: any) => {
    try {
      ...
      doc.text("FlexHavens Invest", 14, 18);
      doc.setFontSize(13);
      doc.text("Mortgage Statement", 14, 26);
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ["Mortgage plan", `${m.planName} · ${m.paymentFrequency} payments`],
      ...
      doc.text(
        "FlexHavens Real Estate Development Ltd. · Abuja, FCT, Nigeria — generated for informational purposes only.",
        14,
  ...
  const renderCard = (m: any) => (
    <div key={m.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-2">
            <div>
              <p className="font-bold text-[#1e3a5f]">{m.propertyName}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm mb-3">
            <div>
              ...
              <p className="font-semibold text-[#1e3a5f]">{formatCurrency(m.totalPayable)}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400">Down Payment</p>
              <p className="font-semibold text-[#b07d52]">{formatCurrency(m.downPaymentAmount)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-bold text-[#1e3a5f]">{formatCurrency(m.remainingBalance)}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400">{m.paymentFrequency === "yearly" ? "Yearly" : "Monthly"} Installment</p>
              <p className="font-semibold text-[#c8956c]">{formatCurrency(m.installmentAmount)}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400">Next Payment</p>
              <p className="font-semibold text-[#1e3a5f]">{m.nextPaymentAt ? formatDate(m.nextPaymentAt) : "—"}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400">Start Date</p>
              <p className="font-semibold text-[#1e3a5f]">{m.startDate ? formatDate(m.startDate) : "—"}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400">Est. Completion</p>
              <p className="font-semibold text-[#1e3a5f]">{m.completionDate ? formatDate(m.completionDate) : "—"}</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <ProgressBar value={m.progress} className="flex-1" />
            <span className="text-xs font-bold text-[#1e3a5f]">{m.progress}%</span>
          </div>

          {m.adminNote && (
            <p className="text-xs text-gray-500 mt-2 bg-white border border-gray-200 rounded-lg px-3 py-2">
              <span className="font-semibold text-[#1e3a5f]">Admin note:</span> {m.adminNote}
            </p>
          )}
          ...
          {m.status === "suspended" && (
            <p className="text-xs text-red-500 mt-2">This mortgage is suspended. Please contact support.</p>
          )}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Button size="sm" variant="outline" className="border-gray-300 text-[#1e3a5f]" onClick={() => setDetailTarget(m)}>
            ...
            <Button size="sm" variant="outline" className="border-gray-300 text-[#1e3a5f]" onClick={() => downloadStatement(m)}>
  ...
  const viewButton = (id: View, label: string, count?: number) => (
    <button
      onClick={() => setView(id)}
      className={`px-4 py-2 rounded-lg text-sm font-medium transition ${view === id ? "bg-[#1e3a5f] text-white" : "text-gray-500 hover:text-[#1e3a5f]"}`}
    >
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title="My Mortgages"
        subtitle="Mortgage-based property purchases and wallet payments"
        action={
          <div className="flex flex-wrap gap-2">
            {viewButton("mortgages", "Mortgages", open.length)}
      ...
      >
        {view === "history" ? (
          historyQuery.isLoading ? (
            ...
            <EmptyState icon={Receipt} title="No payments yet" text="Your mortgage payments will appear here with full balance details and receipt numbers." />
          ) : (
                  ...
                  {payments.map((p) => (
                    <tr key={p.id} className="hover:bg-[#faf8f5] transition">
                      ...
                      <td className="py-3.5 pr-4">
                        <p className="font-semibold text-[#1e3a5f]">{p.propertyName}</p>
                        <p className="text-xs text-gray-400">{p.planName}{p.method === "manual_adjustment" ? " · admin adjustment" : ""}</p>
                      </td>
                      <td className="py-3.5 pr-4 font-bold text-[#c8956c]">{formatCurrency(p.amount)}</td>
                      ...
                      <td className="py-3.5 pr-4 font-semibold text-[#1e3a5f]">{formatCurrency(p.remainingBalanceAfter)}</td>
                      <td className="py-3.5 pr-4 font-mono text-xs text-gray-500">{p.reference}</td>
                      <td className="py-3.5 pr-4 font-mono text-xs text-[#b07d52]">{p.receiptNo}</td>
          ...
          applications.length === 0 ? (
            <EmptyState
              ...
              text="When you apply for a mortgage, it will appear here until our team approves it."
              action={
                <a href="/mortgage#properties" className="inline-block">
                  <Button className="bg-[#1e3a5f]">Apply for a Mortgage</Button>
                </a>
              }
            />
          ) : (
          ...
          <EmptyState
            ...
            text="Browse the catalog and choose 'Buy with Mortgage' on any eligible property to get started."
            action={
              <a href="/mortgage#properties" className="inline-block">
                <Button className="bg-[#1e3a5f]">Browse Properties</Button>
              </a>
            }
          />
        ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" onClick={() => setDetailTarget(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <FileText className="w-5 h-5 text-[#c8956c]" /> Mortgage Details
              </h3>
            ...
            <div className="p-6 space-y-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-bold text-[#1e3a5f] text-lg">{detailTarget.propertyName}</p>
                  <p className="text-xs text-gray-400">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Total Contract</p><p className="font-semibold text-[#1e3a5f]">{formatCurrency(detailTarget.totalPayable)}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Down Payment</p><p className="font-semibold text-[#b07d52]">{formatCurrency(detailTarget.downPaymentAmount)}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Paid So Far</p><p className="font-semibold text-green-600">{formatCurrency(detailTarget.amountPaid)}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Remaining</p><p className="font-bold text-[#1e3a5f]">{formatCurrency(detailTarget.remainingBalance)}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Installment</p><p className="font-semibold text-[#c8956c]">{formatCurrency(detailTarget.installmentAmount)}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Duration</p><p className="font-semibold text-[#1e3a5f]">{detailTarget.durationMonths} months</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Next Payment</p><p className="font-semibold text-[#1e3a5f]">{detailTarget.nextPaymentAt ? formatDate(detailTarget.nextPaymentAt) : "—"}</p></div>
                <div className="bg-[#faf8f5] rounded-lg p-3"><p className="text-[11px] text-gray-400">Est. Completion</p><p className="font-semibold text-[#1e3a5f]">{detailTarget.completionDate ? formatDate(detailTarget.completionDate) : "—"}</p></div>
              </div>

              <div className="flex items-center gap-3">
                <ProgressBar value={detailTarget.progress} className="flex-1" />
                <span className="text-xs font-bold text-[#1e3a5f]">{detailTarget.progress}%</span>
              </div>

              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <CalendarClock className="w-4 h-4 text-[#c8956c]" /> Estimated Repayment Schedule
                </p>
                    ...
                    <tbody className="divide-y">
                      {scheduleFor(detailTarget).map((r) => (
                        <tr key={r.n} className={r.n === 0 ? "bg-[#c8956c]/5" : ""}>
                          <td className="py-2 px-4 font-medium text-[#1e3a5f]">{r.label}</td>
                          <td className="py-2 px-4 text-gray-500 whitespace-nowrap">{formatDate(r.date)}</td>
                          <td className="py-2 px-4 text-right font-semibold text-[#1e3a5f]">{formatCurrency(r.amount)}</td>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Receipt className="w-4 h-4 text-[#c8956c]" /> Payments on This Mortgage
                </p>
                {paymentsFor(detailTarget.id).length === 0 ? (
                  <p className="text-sm text-gray-400 bg-[#faf8f5] rounded-lg px-4 py-3">No payments recorded yet.</p>
                ) : (
                      ...
                      <tbody className="divide-y">
                        {paymentsFor(detailTarget.id).map((p) => (
                          <tr key={p.id}>
                            ...
                            <td className="py-2 px-4 font-semibold text-[#1e3a5f]">{formatCurrency(p.amount)}</td>
                            <td className="py-2 px-4 text-gray-500">Remaining {formatCurrency(p.remainingBalanceAfter)}</td>
                            <td className="py-2 px-4 font-mono text-xs text-[#b07d52]">{p.receiptNo}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                <Button variant="outline" className="border-gray-300 text-[#1e3a5f]" onClick={() => downloadStatement(detailTarget)}>
      ...
      {payTarget && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setPayTarget(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <Wallet className="w-5 h-5 text-[#c8956c]" /> Mortgage Payment
              </h3>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-[#faf8f5] rounded-xl p-4 text-sm space-y-1.5">
                <p className="font-bold text-[#1e3a5f]">{payTarget.propertyName}</p>
                <p className="text-xs text-gray-400 font-mono">{payTarget.reference}</p>
                <div className="flex justify-between pt-1"><span className="text-gray-500">Remaining balance</span><span className="font-semibold text-[#1e3a5f]">{formatCurrency(payTarget.remainingBalance)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Your wallet balance</span><span className="font-semibold text-[#b07d52]">{formatCurrency(walletBalance)}</span></div>
              </div>
              <div>
                ...
                <input
                  ...
                  className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-3 text-lg font-bold text-[#1e3a5f] focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]"
                />
              ...
              <Button
                className="w-full bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87] py-6 text-base"

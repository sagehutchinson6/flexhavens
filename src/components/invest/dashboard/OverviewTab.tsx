    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          label="Overall ROI"
          value={`${overallRoi}%`}
          sub={`${formatCurrency(stats?.totalInvested ?? 0)} total invested`}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        >
          {activePortfolio.length > 0 ? (
            ...
            <EmptyState
              ...
              text="Make your first investment to see your portfolio growth chart."
              action={
        ...
        <SectionCard title="Allocation" subtitle="By investment plan" className="lg:col-span-2">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <SectionCard
          ...
          className="lg:col-span-3"
          action={
            <Button variant="ghost" size="sm" onClick={() => setTab("portfolio")} className="text-[#1e3a5f]">
          ...
          {activePortfolio.length > 0 ? (
            <div className="space-y-4">
              {activePortfolio.slice(0, 3).map((inv: any) => (
                <div key={inv.id} className="bg-[#faf8f5] rounded-xl p-4">
                  ...
                  <div className="flex justify-between mt-1.5 text-xs">
                    <span className="text-[#b07d52] font-semibold">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <SectionCard title="Quick Actions">
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => setTab("deposit")} className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87] h-11">
                <DollarSign className="w-4 h-4 mr-2" /> Deposit
              </Button>
              <Button onClick={() => setTab("withdraw")} variant="outline" className="border-[#1e3a5f] text-[#1e3a5f] h-11">
                <Wallet className="w-4 h-4 mr-2" /> Withdraw
              </Button>
              <Button onClick={() => setTab("invest")} variant="outline" className="border-[#c8956c] text-[#b07d52] h-11">
          ...
          <SectionCard
            title="Notifications"
            action={
              <Button variant="ghost" size="sm" onClick={() => setTab("notifications")} className="text-[#1e3a5f]">
                  ...
                  <div key={n.id} className="flex gap-3">
                    <span
                      className={`w-2 h-2 rounded-full mt-2 shrink-0 ${
                        n.isRead === "no" ? "bg-[#c8956c]" : "bg-gray-200"
                      }`}
                    />
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-[#1e3a5f] truncate">{n.title}</p>
      ...
      <SectionCard
        ...
        action={
          <Button variant="ghost" size="sm" onClick={() => setTab("transactions")} className="text-[#1e3a5f]">
            View All <ArrowRight className="w-4 h-4 ml-1" />
          </Button>
        }
      >
        {transactions && transactions.length > 0 ? (
            ...
            {transactions.slice(0, 5).map((tx: any) => (
              <div key={tx.id} className="flex items-center justify-between py-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#1e3a5f] truncate">{tx.description}</p>
          ...
          <EmptyState
            ...
            text="Deposits, investments, and earnings will appear here."
          />
        )}
      </SectionCard>

      <p className="text-xs text-gray-400 text-center">
        Need help? Visit the <Link to="/faq" className="text-[#1e3a5f] underline">FAQ</Link> or contact support.
      </p>
    </div>
  );
}

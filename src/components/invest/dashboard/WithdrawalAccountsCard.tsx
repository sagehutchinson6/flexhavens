...
const METHOD_META: Record<Method, { label: string; icon: any }> = {
  ...
  opay: { label: "OPay", icon: Smartphone },
...
export default function WithdrawalAccountsCard() {
  ...
  const handleSave = () => {
    ...
    if (form.method === "opay" && (!payload.accountName || !payload.accountNumber)) {
      toast.error("Account name and OPay account / phone number are required.");
      return;
    }
  ...
  return (
    <SectionCard
      ...
      subtitle="Saved bank, OPay and crypto destinations for faster withdrawals"
      action={
    ...
    >
      {list.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm font-semibold text-[#1e3a5f]">No saved accounts yet</p>
          <p className="text-xs text-gray-400 mt-1 max-w-sm mx-auto">
            Save your bank, OPay or crypto account once and select it whenever you withdraw —
          ...
          {list.map((a) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 py-3.5">
                <div className="w-10 h-10 rounded-xl bg-[#1e3a5f]/[0.06] flex items-center justify-center shrink-0">
                  <Meta.icon className="w-5 h-5 text-[#1e3a5f]" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-[#1e3a5f] flex items-center gap-2">
                    ...
                    {a.isDefault === "yes" && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#c8956c] shrink-0">
                        <Star className="w-3 h-3 fill-[#c8956c]" /> Default
                      </span>
                    )}
                  </p>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {a.isDefault !== "yes" && (
                    <button
                      ...
                      className="p-2 rounded-lg text-gray-400 hover:text-[#c8956c] hover:bg-gray-100 transition"
                    >
                  ...
                  <button
                    onClick={() => openEdit(a)}
                    title="Edit"
                    className="p-2 rounded-lg text-gray-400 hover:text-[#1e3a5f] hover:bg-gray-100 transition"
                  >
          ...
          <div className="space-y-4 mt-2">
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border-2 text-center transition ${
                        form.method === m ? "border-[#1e3a5f] bg-[#1e3a5f]/[0.04]" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <Meta.icon className={`w-4 h-4 mx-auto mb-1 ${form.method === m ? "text-[#1e3a5f]" : "text-gray-400"}`} />
                      <p className="text-[11px] font-semibold text-[#1e3a5f]">{Meta.label}</p>
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder={form.method === "bank" ? "e.g. My GTBank account" : form.method === "opay" ? "e.g. My OPay" : "e.g. USDT wallet"}
                className="mt-1.5"
                maxLength={100}
              />
            </div>
            ...
            {form.method === "opay" && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label htmlFor="wa-opayname">Account Name</Label>
                  <Input id="wa-opayname" value={form.accountName} onChange={(e) => setForm((f) => ({ ...f, accountName: e.target.value }))} placeholder="Full name" className="mt-1.5" maxLength={150} />
                </div>
                <div>
                  <Label htmlFor="wa-opaynum">OPay Account / Phone</Label>
                  <Input id="wa-opaynum" value={form.accountNumber} onChange={(e) => setForm((f) => ({ ...f, accountNumber: e.target.value }))} placeholder="e.g. 08012345678" className="mt-1.5" maxLength={40} />
                </div>
              </div>
            )}
            {form.method === "crypto" && (
            ...
            <Button onClick={handleSave} disabled={save.isPending} className="w-full bg-[#1e3a5f]">

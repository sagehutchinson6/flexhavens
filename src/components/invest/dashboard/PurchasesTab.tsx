            ...
            <li key={stage.key} className="ml-5 relative">
              <span className="absolute -left-[27px] top-0.5">
                {done ? (
                  ...
                  <Clock className="w-5 h-5 text-[#c8956c] bg-white rounded-full" />
                ) : (
              ...
              <p
                className={`text-sm font-medium ${
                  current ? "text-[#1e3a5f]" : done ? "text-gray-700" : "text-gray-400"
                }`}
              >
                {stage.label}
                {current && (
                  <span className="ml-2 text-xs font-semibold text-[#c8956c]">Current stage</span>
                )}
              </p>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="font-semibold text-[#1e3a5f] font-serif truncate">
              {propertyName}
              {extraItems > 0 && (
          ...
          <div className="text-right flex-shrink-0">
            <p className="font-bold text-[#1e3a5f]">{formatCurrency(Number(order.totalAmount))}</p>
        ...
        <div className="border-t border-gray-100 p-4 sm:p-5 bg-gray-50/50">
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-sm mb-2">
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              ...
              <p className="font-semibold text-[#1e3a5f]">{formatCurrency(Number(order.totalAmount))}</p>
            </div>
            <div className="bg-white rounded-lg border border-gray-200 p-3">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              ...
              <p className="font-semibold text-[#1e3a5f] capitalize">{order.paymentStatus}</p>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {order.documents.map((d: any) => (
                  <a
                    ...
                    href={d.dataUrl}
                    download={d.name}
                    className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-3 py-2 hover:border-[#c8956c] transition"
                  >
          ...
          <div className="mt-4 flex justify-end">
            <Link
              to={trackUrl}
              className="inline-flex items-center gap-1.5 text-sm font-medium text-[#1e3a5f] hover:text-[#c8956c] transition"
            >
...
function MortgagePurchaseCard({ mortgage, onGoToMortgages }: { mortgage: any; onGoToMortgages?: () => void }) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-semibold text-[#1e3a5f] font-serif truncate">{mortgage.propertyName}</p>
        ...
        <div className="text-right flex-shrink-0">
          <p className="font-bold text-[#1e3a5f]">{formatCurrency(Number(mortgage.propertyPrice))}</p>
          <span
      ...
      <div className="mt-4 grid sm:grid-cols-3 gap-3 text-sm">
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <p className="font-semibold text-[#1e3a5f]">{mortgage.progress}%</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-xs text-gray-400">Next Payment Due</p>
          <p className="font-semibold text-[#1e3a5f] flex items-center gap-1">
            <CalendarClock className="w-3.5 h-3.5 text-[#c8956c]" />
        ...
        <SectionCard title="My Property Purchases">
          <EmptyState
            ...
            action={
              <Link
                to="/#catalog"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#1e3a5f] text-white text-sm font-medium hover:bg-[#2d5a87] transition"
              >
                Browse Properties
              </Link>
            }

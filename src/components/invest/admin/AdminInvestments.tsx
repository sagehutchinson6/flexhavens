...
export default function AdminInvestments() {
  ...
  const approve = trpc.investAdmin.approveInvestment.useMutation({ onSuccess: onSuccess("Investment approved and activated"), onError });
  const reject = trpc.investAdmin.rejectInvestment.useMutation({ onSuccess: onSuccess("Investment rejected, principal refunded"), onError });
  const setStatus = trpc.investAdmin.setInvestmentStatus.useMutation({ onSuccess: onSuccess("Investment updated"), onError });
  ...
  return (
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Investor, project or ID..."
              className="pl-9 w-52"
            />
          </div>
    ...
    >
      {items && items.length > 0 ? (
          ...
          {items.map((inv: any) => (
            <div key={inv.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {inv.status === "active" && (
                  <>
                    <Button size="sm" disabled={busy} onClick={() => creditNow.mutate({ investmentId: inv.id })} className="bg-[#1e3a5f] h-8 text-xs">
                      <DollarSign className="w-3.5 h-3.5 mr-1" /> Credit Profit Now
                    </Button>
                    <Button size="sm" variant="outline" disabled={busy} className="border-[#1e3a5f] text-[#1e3a5f] h-8 text-xs"
                      onClick={() => {
        ...
        <EmptyState icon={TrendingUp} title="No investments found" text="Investments matching this filter will appear here." />
      )}
    </SectionCard>
  );
}

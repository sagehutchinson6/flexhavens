    ...
    <div className="space-y-6">
      ...
      <SectionCard title="Monthly ROI Engine" subtitle="Credits due profits and returns matured principal">
        <div className="max-w-2xl space-y-4">
          <div className="bg-[#faf8f5] rounded-xl p-5 text-sm text-gray-600 leading-relaxed">
            <p className="font-semibold text-[#1e3a5f] mb-2">How monthly ROI works</p>
          ...
          <Button onClick={() => settle.mutate()} disabled={settle.isPending} className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]">
                  ...
                  <tr key={p.id} className="hover:bg-[#faf8f5] transition">
                    <td className="py-3.5 pr-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-[#1e3a5f]">{p.investorName}</span>
                      </div>
                    </td>
            ...
            {(investors ?? []).map((inv: any) => (
              <button
                ...
                className={`w-full text-left p-3.5 rounded-xl border-2 transition flex items-center justify-between gap-3 ${
                  selected?.id === inv.id ? "border-[#1e3a5f] bg-[#1e3a5f]/[0.04]" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                ...
                <p className="font-bold text-[#1e3a5f] text-sm shrink-0">{formatCurrency(inv.walletBalance)}</p>
              </button>
            ))}
          </div>
        </SectionCard>
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                <Button
                  ...
                  variant="outline"
                  disabled={recalculate.isPending}
                  className="border-[#c8956c] text-[#b07d52]"
                  ...
                  {detail.transactions.map((tx: any) => (
                    <div key={tx.id} className="py-3 flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        {tx.status === "completed" && (
                          <button
                            className="text-xs font-semibold text-[#b07d52] hover:underline flex items-center gap-1"
                            onClick={() => {

...
export default function AdminReportsPro() {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const doc = new jsPDF();
      const title = `FlexHavens ${reportOptions.find((r) => r.id === reportType)?.label} Report`;
      doc.setFontSize(18);
      ...
      doc.save(`flexhavens-${reportType}-report-${new Date().toISOString().slice(0, 10)}.pdf`);
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `flexhavens-${reportType}-report-${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar dataKey="propertyRevenue" name="Property Revenue" fill="#c8956c" radius={[4, 4, 0, 0]} />
              <Bar dataKey="deposits" name="Investor Deposits" fill="#1e3a5f" radius={[4, 4, 0, 0]} />
      ...
      <SectionCard title="Downloadable Reports" subtitle="Generate PDF or Excel reports for any dataset">
        <div className="flex flex-wrap items-end gap-4">
              ...
              {reportOptions.map((r) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-semibold transition ${
                    reportType === r.id ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {r.label}
                </button>
          ...
          <div className="flex gap-3">
            <Button onClick={exportPdf} disabled={exporting} className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]">
              <FileText className="w-4 h-4 mr-2" />
              {exporting ? "Generating..." : "Export PDF"}
            </Button>
            <Button onClick={exportExcel} disabled={exporting} variant="outline" className="border-[#1e3a5f] text-[#1e3a5f]">

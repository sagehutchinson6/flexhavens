...
export default function AdminDeletionFeedback() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-[#1e3a5f] font-serif">Account Deletion Feedback</h3>
      ...
      {feedbackQuery.isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="font-semibold text-[#1e3a5f]">No deletion feedback yet</p>
          ...
          {rows.map((r) => (
            <div key={r.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100">
              <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                <div>
                  <p className="font-bold text-[#1e3a5f]">{r.name}</p>
                  <p className="text-xs text-gray-400">{r.email}</p>
                </div>
                <span className="text-xs text-gray-500">{fmtDate(r.createdAt)}</span>
              </div>
              <p className="text-sm">
                <span className="font-semibold text-[#b07d52]">Reason:</span>{" "}

...
const inputCls =
  "border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30 bg-white";
...
const TYPE_COLORS: Record<string, string> = {
  property_inspection: "#1e3a5f",
  virtual_tour: "#0ea5e9",
  office_meeting: "#c8956c",
...
export default function AdminAppointments() {
  ...
  const [filters, setFilters] = useState({ status: "", type: "", assignedAdminId: "", search: "", dateFrom: "", dateTo: "" });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex rounded-full border border-gray-200 overflow-hidden bg-white">
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 transition ${view === "list" ? "bg-[#1e3a5f] text-white" : "text-[#1e3a5f]"}`}
          >
            <ListChecks className="w-3.5 h-3.5" /> List
          </button>
          <button
            onClick={() => setView("calendar")}
            className={`flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 transition ${view === "calendar" ? "bg-[#1e3a5f] text-white" : "text-[#1e3a5f]"}`}
          >
        ...
        {view === "calendar" && (
          <>
            <div className="flex rounded-full border border-gray-200 overflow-hidden bg-white">
              {(["day", "week", "month"] as const).map((m) => (
                <button
                  ...
                  className={`text-xs font-semibold px-3 py-2 capitalize transition ${calMode === m ? "bg-[#c8956c] text-white" : "text-[#1e3a5f]"}`}
                >
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => moveCursor(-1)} className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-[#1e3a5f] hover:bg-gray-50">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={() => setCursor(new Date())} className="text-xs font-semibold text-[#1e3a5f] px-3 py-2 rounded-full border border-gray-200 bg-white hover:bg-gray-50">
                Today
              </button>
              <button onClick={() => moveCursor(1)} className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-[#1e3a5f] hover:bg-gray-50">
            ...
            <p className="text-sm font-bold text-[#1e3a5f] font-serif">
              {calMode === "month"
        ...
        <Button
          variant="outline"
          className="ml-auto text-[#1e3a5f] border-[#1e3a5f]/30"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 grid grid-cols-2 md:grid-cols-6 gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setFilters({ status: "", type: "", assignedAdminId: "", search: "", dateFrom: "", dateTo: "" })} className="w-9 h-9 shrink-0 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:text-[#1e3a5f]" title="Reset filters">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#faf8f5] text-left text-xs uppercase tracking-wider text-gray-500">
                ...
                {appts.map((a) => (
                  <tr key={a.id} className="hover:bg-[#faf8f5]">
                    <td className="px-4 py-3">
                      <p className="font-semibold text-[#1e3a5f]">{a.customerName}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button title="View details" onClick={() => setSelected(a)} className="w-8 h-8 rounded-lg bg-[#1e3a5f]/5 text-[#1e3a5f] flex items-center justify-center hover:bg-[#1e3a5f]/10">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-7 gap-1">
                {monthCells.map((d, i) => {
                  ...
                  const items = byDay.get(dayKey(d)) ?? [];
                  return (
                    <div key={i} className={`min-h-[90px] rounded-lg border p-1.5 ${inMonth ? "bg-white border-gray-200" : "bg-gray-50 border-gray-100"} ${isToday ? "ring-2 ring-[#c8956c]" : ""}`}>
                      <p className={`text-[11px] font-semibold mb-1 ${inMonth ? "text-[#1e3a5f]" : "text-gray-300"}`}>{d.getDate()}</p>
          ...
          {calMode === "week" && (
            <div className="grid grid-cols-7 gap-2">
              {weekDays.map((d) => (
                <div key={dayKey(d)}>
                  <p className={`text-center text-xs font-bold mb-2 ${dayKey(d) === dayKey(new Date()) ? "text-[#c8956c]" : "text-[#1e3a5f]"}`}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6">
            ...
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h3 className="font-serif text-xl font-bold text-[#1e3a5f]">{appointmentTypeLabel(selected.type)}</h3>
              <StatusPill status={selected.status} />
            </div>
            <p className="text-xs text-gray-400 font-mono mb-4">{selected.appointmentRef}</p>

            <div className="space-y-2 text-sm text-gray-600">
              <p className="flex items-center gap-2"><UserRound className="w-4 h-4 text-[#c8956c]" /> <strong className="text-[#1e3a5f]">{selected.customerName}</strong></p>
              <p className="flex items-center gap-2"><Mail className="w-4 h-4 text-[#c8956c]" /> {selected.email}</p>
              <p className="flex items-center gap-2"><Phone className="w-4 h-4 text-[#c8956c]" /> {selected.phone}</p>
              <p className="flex items-center gap-2"><CalendarDays className="w-4 h-4 text-[#c8956c]" /> {fmtDate(selected.preferredAt)} at {fmtTime(selected.preferredAt)} (WAT)</p>
              <p className="flex items-center gap-2"><Clock className="w-4 h-4 text-[#c8956c]" /> {selected.durationMinutes} minutes</p>
              {selected.propertyName && <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#c8956c]" /> {selected.propertyName}</p>}
              {selected.location && <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#c8956c]" /> Location: {selected.location}</p>}
              {selected.meetingLink && (
                <p className="flex items-center gap-2 break-all">
                  <Link2 className="w-4 h-4 text-[#c8956c] shrink-0" />
                  <a href={selected.meetingLink} target="_blank" rel="noreferrer" className="text-[#1e3a5f] underline">{selected.meetingLink}</a>
                </p>
              )}
              <p className="flex items-center gap-2"><UserRound className="w-4 h-4 text-[#c8956c]" /> Assigned: {selected.assignedAdminName ?? "—"}</p>
            </div>

            {selected.notes && (
              <div className="mt-4 bg-[#faf8f5] border border-gray-200 rounded-lg px-3 py-2">
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl max-w-md w-full shadow-2xl p-6 space-y-3">
        ...
        <h3 className="font-serif text-lg font-bold text-[#1e3a5f]">{title}</h3>
        {children}
      </div>
    </div>
  );

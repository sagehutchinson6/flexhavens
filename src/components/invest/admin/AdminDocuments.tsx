...
const statusStyles: Record<string, string> = {
  ...
  generated: "bg-[#1e3a5f]/10 text-[#1e3a5f]",
...
function UploadModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    if (!email.includes("@")) return toast.error("Enter the investor's email address.");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
        <h3 className="font-serif text-xl font-bold text-[#1e3a5f]">Upload Document</h3>
        <p className="text-sm text-gray-500 mt-1 mb-5">Attach an official document to an investor's Document Center.</p>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Investor Email <span className="text-red-500">*</span></label>
            <input
              value={email}
          ...
          <div className={`border-2 border-dashed rounded-xl p-4 ${file ? "border-green-300 bg-green-50/50" : "border-gray-200"}`}>
            {file ? (
              ...
              <label className="cursor-pointer flex flex-col items-center gap-2 py-2 text-gray-500">
                <Upload className="w-6 h-6 text-[#c8956c]" />
        ...
        <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-gray-100">
          ...
          <button
            onClick={submit}
            disabled={busy}
            className="px-5 py-2.5 rounded-lg bg-[#1e3a5f] text-white text-sm font-medium hover:bg-[#2d5a87] transition disabled:opacity-50 flex items-center gap-2"
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h3 className="font-serif text-xl font-bold text-[#1e3a5f]">Download History</h3>
        ...
        {historyQuery.isLoading ? (
          <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#1e3a5f]" /></div>
        ) : rows.length === 0 ? (
...
export default function AdminDocuments() {
  ...
  const replaceMutation = trpc.document.replaceDocument.useMutation({
    onSuccess: () => { toast.success("Document replaced — version bumped and investor notified."); invalidate(); },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-serif text-2xl font-bold text-[#1e3a5f] flex items-center gap-2">
            <FolderOpen className="w-6 h-6 text-[#c8956c]" />
            Document Management
          </h2>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-lg bg-[#1e3a5f] text-white text-sm font-medium hover:bg-[#2d5a87] transition flex items-center gap-2"
        >
      ...
      {/* Filters */}
      <SectionCard title="Search & Filter" subtitle="Find documents by name, reference, property, investor, category or status.">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search name, reference, property, investor, receipt no…"
              className="w-full border border-gray-300 rounded-lg pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30"
            />
          </div>
          <select
            value={category}
            onChange={(e) => setCategory(e.target.value)}
            className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30"
          >
      ...
      <SectionCard title={`All Documents (${filtered.length})`} subtitle="Preview, download, replace, re-status or remove any document.">
        {docsQuery.isLoading ? (
          <div className="flex justify-center py-14"><Loader2 className="w-7 h-7 animate-spin text-[#1e3a5f]" /></div>
        ) : filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto -mx-5 px-5">
            <table className="w-full text-sm min-w-[980px]">
              ...
              <tbody>
                {filtered.map((d) => (
                  <tr key={d.id} className="border-b border-gray-50 hover:bg-[#faf8f5]/60">
                    <td className="py-3 pr-4">
                      <div className="flex items-start gap-2.5">
                        <FileText className="w-4 h-4 text-[#c8956c] mt-0.5 shrink-0" />
                        <div>
                          <p className="font-medium text-gray-800 leading-snug">{d.name}</p>
                          <p className="text-xs text-gray-400 mt-0.5">
                            {d.docRef} · {d.docType} · v{d.version}
                            {d.source === "generated" && <span className="ml-1 text-[#1e3a5f]/60">(auto)</span>}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-4">
                      <button
                        ...
                        className="flex items-center gap-1 text-gray-600 hover:text-[#1e3a5f] transition"
                    ...
                    <td className="py-3">
                      <div className="flex items-center justify-end gap-1">
                        {busyId === d.id ? (
                          <Loader2 className="w-4 h-4 animate-spin text-[#1e3a5f] mx-2" />
                        ) : (
                          <>
                            <button onClick={() => openDoc(d, "preview")} title="Preview" className="p-1.5 rounded-lg hover:bg-[#1e3a5f]/10 text-gray-500 hover:text-[#1e3a5f] transition">
                              <Eye className="w-4 h-4" />
                            </button>
                            <button onClick={() => openDoc(d, "download")} title="Download" className="p-1.5 rounded-lg hover:bg-[#1e3a5f]/10 text-gray-500 hover:text-[#1e3a5f] transition">
                              <Download className="w-4 h-4" />
                            </button>
                            <button onClick={() => setReplaceTarget(d)} title="Replace file (new version)" className="p-1.5 rounded-lg hover:bg-[#1e3a5f]/10 text-gray-500 hover:text-[#1e3a5f] transition">
      ...
      {replaceTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h3 className="font-serif text-lg font-bold text-[#1e3a5f]">Replace Document</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <label className="px-4 py-2 rounded-lg bg-[#1e3a5f] text-white text-sm font-medium hover:bg-[#2d5a87] transition cursor-pointer flex items-center gap-2">
                <Upload className="w-4 h-4" />
                Choose New File
                <input
                  type="file"

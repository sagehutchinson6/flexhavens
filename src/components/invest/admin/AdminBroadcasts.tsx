...
export default function AdminBroadcasts() {
  ...
  const inputCls = "w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40";
      ...
      <SectionCard title="Send a Broadcast" subtitle="Reach user groups with in-app notifications and branded emails">
        ...
        <div className="mt-4 flex items-center justify-between flex-wrap gap-3">
          <label className="flex items-center gap-2.5 text-sm text-gray-600 cursor-pointer">
            <input type="checkbox" checked={sendEmail} onChange={(e) => setSendEmail(e.target.checked)} className="w-4 h-4 accent-[#1e3a5f]" />
            Also send as email (respects user preferences)
          </label>
          <Button onClick={handleSend} disabled={send.isPending} className="bg-[#1e3a5f] hover:bg-[#2d5a87] text-white">
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-[#1e3a5f]">{b.title}</p>
                      <span className={`text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded ${b.kind === "emergency" ? "bg-red-50 text-red-600" : "bg-[#1e3a5f]/5 text-[#1e3a5f]"}`}>
                  ...
                  <div className="flex items-center gap-4 text-center shrink-0">
                    <div>
                      <p className="text-lg font-bold text-[#1e3a5f]">{b.recipientCount}</p>

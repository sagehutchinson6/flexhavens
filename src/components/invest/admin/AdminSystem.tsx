...
const notifIcons: Record<string, { icon: React.ElementType; classes: string }> = {
  ...
  order: { icon: ShoppingBag, classes: "bg-[#c8956c]/15 text-[#b07d52]" },
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <SectionCard
        ...
        action={
          unread > 0 ? (
            <Button variant="outline" size="sm" onClick={() => markAll.mutate()} className="border-[#1e3a5f] text-[#1e3a5f]">
    ...
    <SectionCard
      ...
      action={
        <div className="flex gap-2">
          <button onClick={() => setView("audit")} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition ${view === "audit" ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            Admin Audit Log
          </button>
          <button onClick={() => setView("activity")} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition ${view === "activity" ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            Investor Activity
          </button>
        </div>
      }
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody className="divide-y">
                {audit.map((log: any) => (
                  <tr key={log.id} className="hover:bg-[#faf8f5] transition">

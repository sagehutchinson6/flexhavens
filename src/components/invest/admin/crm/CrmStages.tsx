  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-bold text-[#1e3a5f] font-serif">Pipeline Stages</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            Add, rename, recolor, reorder or remove stages. Stages in use by leads cannot be removed.
          </p>
        </div>
        <Button size="sm" onClick={() => setForm({ label: "", color: "#3b82f6", kind: "open" })} className="bg-[#1e3a5f] hover:bg-[#2d5a87]">
        ...
        {stages.map((s, i) => (
          <div key={s.id} className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[#1e3a5f] truncate">{s.label}</p>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button title="Edit" onClick={() => setForm({ id: s.id, label: s.label, color: s.color, kind: s.kind })} className="w-7 h-7 rounded-lg bg-[#1e3a5f]/5 text-[#1e3a5f] flex items-center justify-center hover:bg-[#1e3a5f]/10">
      ...
      {form && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-sm w-full shadow-2xl p-6">
            ...
            <h3 className="font-serif text-lg font-bold text-[#1e3a5f] mb-4">{form.id ? "Edit Stage" : "New Stage"}</h3>

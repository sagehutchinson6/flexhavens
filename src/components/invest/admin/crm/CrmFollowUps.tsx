        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition ${
              filter === f.key ? "bg-[#1e3a5f] text-white border-[#1e3a5f]" : "bg-white text-[#1e3a5f] border-gray-200 hover:border-[#c8956c]"
            }`}
          >
            {f.label}
          </button>
              ...
              return (
                <div key={f.id} className={`flex items-center gap-3 px-4 py-3 ${overdue ? "bg-red-50/50" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold truncate ${f.status !== "pending" ? "line-through text-gray-400" : "text-[#1e3a5f]"}`}>
                      {f.title}
                    </p>
                  ...
                  {f.lead && (
                    <button
                      ...
                      className="text-xs text-[#1e3a5f] font-semibold hover:underline flex items-center gap-1 shrink-0"
                    >

...
const NAVY = "#1e3a5f";
const COPPER = "#c8956c";
const PIE_COLORS = [NAVY, COPPER, "#2d5a87", "#b07d52", "#64748b", "#0ea5e9", "#16a34a", "#dc2626"];

function Card({ icon: Icon, label, value, sub, accent }: { icon: typeof Users; label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div className={`rounded-2xl p-5 border transition hover:shadow-md ${accent ? "bg-gradient-to-br from-[#c8956c] to-[#b07d52] border-transparent text-white" : "bg-white border-gray-200"}`}>
      <div className="flex items-start justify-between">
        ...
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${accent ? "bg-white/20" : "bg-[#1e3a5f]/5"}`}>
          <Icon className={`w-4 h-4 ${accent ? "text-white" : "text-[#1e3a5f]"}`} />
        </div>
      </div>
    </div>
  );
...
function ChartCard({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-white rounded-2xl border border-gray-200 p-5 ${className ?? ""}`}>
      <h4 className="font-bold text-[#1e3a5f] text-sm mb-4">{title}</h4>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Card icon={TrendingUp} label="Top Investment" value={cards.mostPopularInvestmentPlan ?? "—"} sub="most popular plan" />
          ...
          <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
            {charts.funnel.map((f) => (
              <div key={f.key} className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold text-[#1e3a5f] w-8 text-right">{f.count}</span>
              </div>
            ))}
          </div>
        </ChartCard>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-full border transition ${
                  view === v.id
                    ...
                    : "bg-white text-[#1e3a5f] border-gray-200 hover:border-[#c8956c]"
                }`}
              >

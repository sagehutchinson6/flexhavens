    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell className="w-4 h-4 text-[#c8956c]" />
          <p className="font-bold text-[#1e3a5f] font-serif">My Notifications</p>
          {unread > 0 && (
      ...
      {rows.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100">
          {rows.map((n) => (
            <div key={n.id} className={`px-5 py-3.5 flex items-start gap-3 ${n.isRead === "no" ? "bg-[#faf8f5]" : ""}`}>
              <span className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${n.isRead === "no" ? "bg-[#c8956c]" : "bg-gray-200"}`} />
              <div className="flex-1 min-w-0">
                <p className={`text-sm ${n.isRead === "no" ? "font-semibold text-[#1e3a5f]" : "text-gray-600"}`}>{n.title}</p>
              ...
              {n.isRead === "no" && (
                <button
                  ...
                  className="w-8 h-8 rounded-lg bg-[#1e3a5f]/5 text-[#1e3a5f] flex items-center justify-center hover:bg-[#1e3a5f]/10 shrink-0"
                >

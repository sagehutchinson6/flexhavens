...
const inputCls =
  "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30";
...
export default function CrmLeadProfile({ leadId, onClose }: { leadId: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="absolute inset-y-0 right-0 w-full max-w-3xl bg-[#faf8f5] shadow-2xl overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#1e3a5f] text-white px-6 py-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="w-12 h-12 rounded-full bg-[#c8956c] flex items-center justify-center font-bold text-lg">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="grid sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
              <p className="flex items-center gap-2 text-gray-600"><Mail className="w-4 h-4 text-[#c8956c] shrink-0" /> <a className="text-[#1e3a5f] hover:underline break-all" href={`mailto:${lead.email}`}>{lead.email}</a></p>
              <p className="flex items-center gap-2 text-gray-600"><Phone className="w-4 h-4 text-[#c8956c] shrink-0" /> {lead.phone ?? "—"}</p>
              <p className="flex items-center gap-2 text-gray-600"><MessageCircle className="w-4 h-4 text-[#c8956c] shrink-0" /> {lead.whatsapp ?? "—"}</p>
              <p className="flex items-center gap-2 text-gray-600"><Globe className="w-4 h-4 text-[#c8956c] shrink-0" /> {[lead.city, lead.state, lead.country].filter(Boolean).join(", ") || "—"}</p>
              <p className="flex items-center gap-2 text-gray-600"><Wallet className="w-4 h-4 text-[#c8956c] shrink-0" /> {lead.budgetRange ?? "Budget unknown"}</p>
              <p className="flex items-center gap-2 text-gray-600"><Contact2 className="w-4 h-4 text-[#c8956c] shrink-0" /> Prefers: {lead.preferredContact ?? "—"}</p>
              {lead.interestedProperty && <p className="flex items-center gap-2 text-gray-600 sm:col-span-2"><Building2 className="w-4 h-4 text-[#c8956c] shrink-0" /> Property: <strong className="text-[#1e3a5f]">{lead.interestedProperty}</strong></p>}
              {lead.investmentInterest && <p className="flex items-center gap-2 text-gray-600"><TrendingUp className="w-4 h-4 text-[#c8956c] shrink-0" /> Investment: {lead.investmentInterest}</p>}
              {lead.mortgageInterest && <p className="flex items-center gap-2 text-gray-600"><Landmark className="w-4 h-4 text-[#c8956c] shrink-0" /> Mortgage: {lead.mortgageInterest}</p>}
            </div>
            {lead.notes && (
          ...
          {/* Log activity */}
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h4 className="font-bold text-[#1e3a5f] mb-3 text-sm uppercase tracking-wider">Log Activity</h4>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg border text-xs font-medium transition ${
                      activity.type === t ? "bg-[#1e3a5f] text-white border-[#1e3a5f]" : "border-gray-200 text-gray-600 hover:border-[#c8956c]"
                    }`}
                  >
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-bold text-[#1e3a5f] text-sm uppercase tracking-wider">Follow-ups ({pendingFollowUps.length} pending)</h4>
                  ...
                  return (
                    <div key={f.id} className={`flex items-center gap-3 border rounded-lg px-3 py-2.5 text-sm ${overdue ? "border-red-200 bg-red-50" : "border-gray-200"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`font-medium truncate ${f.status !== "pending" ? "line-through text-gray-400" : "text-[#1e3a5f]"}`}>{f.title}</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-bold text-[#1e3a5f] text-sm uppercase tracking-wider">Appointments ({appointments.length})</h4>
                ...
                {appointments.slice(0, 6).map((a) => (
                  <div key={a.id} className="border border-gray-200 rounded-lg px-3 py-2.5 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-medium text-[#1e3a5f]">{appointmentTypeLabel(a.type)}</p>
                      <span className="text-xs capitalize px-2 py-0.5 rounded-full bg-[#1e3a5f]/5 text-[#1e3a5f]">{a.status.replace(/_/g, " ")}</span>
                    </div>
          ...
          {/* Timeline */}
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h4 className="font-bold text-[#1e3a5f] mb-4 text-sm uppercase tracking-wider">Timeline ({timeline.length})</h4>
            {timeline.length === 0 ? (
                  ...
                  return (
                    <div key={t.id} className="relative">
                      <div className="absolute -left-6 top-0.5 w-5 h-5 rounded-full bg-white border border-gray-200 flex items-center justify-center">
                        <Icon className="w-3 h-3 text-[#c8956c]" />
                      </div>
                      <p className="text-sm text-[#1e3a5f] font-medium">{t.description}</p>
      ...
      {bookOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-md w-full shadow-2xl p-6">
            ...
            <h3 className="font-serif text-xl font-bold text-[#1e3a5f] mb-1">Book Appointment</h3>
            ...
            <Button
              className="w-full mt-4 bg-[#1e3a5f] hover:bg-[#2d5a87]"

    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-2">
          ...
          <Button onClick={() => setCreateOpen(true)} className="bg-[#1e3a5f] hover:bg-[#2d5a87]">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {(data?.leads ?? []).map((l) => {
                ...
                return (
                  <tr key={l.id} className="hover:bg-[#faf8f5] cursor-pointer" onClick={() => setProfileId(l.id)}>
                    <td className="px-4 py-3">
                      <p className="font-semibold text-[#1e3a5f]">{l.fullName}</p>
                    ...
                    <td className="px-4 py-3">
                      {l.assignedAdminName ? (
                        <span className="flex items-center gap-1.5 text-gray-600">
                          <UserRound className="w-3.5 h-3.5 text-[#c8956c]" /> {l.assignedAdminName}
                        </span>
                      ) : (

      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => setActiveId(null)} className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-[#1e3a5f]">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <div className="min-w-0 flex-1">
              <h3 className="font-bold text-[#1e3a5f] truncate">{active.subject}</h3>
                ...
                <div key={m.id} className={`flex ${fromUser ? "justify-start" : "justify-end"}`}>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-[11px] font-semibold ${fromUser ? "text-[#1e3a5f]" : system ? "text-[#c8956c]" : "text-white/80"}`}>
                        {system ? "FlexHavens (System)" : m.senderName}{!fromUser && !system ? " (Admin)" : ""}
                      </span>
                    ...
                    {m.attachmentName && m.attachmentUrl && m.deleted !== "yes" && (
                      <button
                        ...
                        className={`mt-2 flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition ${
                          fromUser || system ? "bg-[#1e3a5f]/5 hover:bg-[#1e3a5f]/10 text-[#1e3a5f]" : "bg-white/10 hover:bg-white/20 text-white"
                        }`}
                      >
                    ...
                    {m.deleted !== "yes" && (
                      <div className={`flex items-center gap-2 mt-1.5 ${fromUser ? "" : "justify-end"}`}>
                        <button
                          ...
                          className={`${fromUser || system ? "text-gray-400 hover:text-[#1e3a5f]" : "text-white/40 hover:text-white/80"} transition`}
                        >
            ...
            <div className="mt-4 border-t border-gray-100 pt-4">
              ...
              <div className="flex items-end gap-2">
                ...
                <Button
                  ...
                  className="bg-[#1e3a5f] hover:bg-[#2d5a87] rounded-full w-10 h-10 p-0 shrink-0"
                >
        ...
        {forwardMsg && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl max-w-md w-full shadow-2xl p-6">
              ...
              <h3 className="font-serif text-lg font-bold text-[#1e3a5f] mb-2">Forward Message</h3>
    ...
    <div className="space-y-4">
        ...
        {QUICK.map((q) => (
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition ${
              filters.quick === q.key ? "bg-[#1e3a5f] text-white border-[#1e3a5f]" : "bg-white text-[#1e3a5f] border-gray-200 hover:border-[#c8956c]"
            }`}
          >
            {q.label}
          </button>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 grid grid-cols-2 md:grid-cols-7 gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setFilters({ search: "", category: "", status: "", priority: "", assignedAdminId: "", dateFrom: "", dateTo: "", quick: "" })} className="w-9 h-9 shrink-0 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:text-[#1e3a5f]" title="Reset">
        ...
        {listQuery.isLoading ? (
          ...
          <div className="divide-y divide-gray-100">
            {convs.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setActiveId(c.id)} className="w-full text-left px-4 py-3.5 hover:bg-[#faf8f5] transition flex items-center gap-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    {c.assignedAdminName && (
                      <p className="text-[11px] text-gray-400 flex items-center gap-1 justify-end mt-0.5">
                        <UserRound className="w-3 h-3 text-[#c8956c]" /> {c.assignedAdminName}
                      </p>
                    )}
                  </div>
                </button>

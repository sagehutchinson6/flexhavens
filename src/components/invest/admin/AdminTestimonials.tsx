...
function Card({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div className={`rounded-2xl p-5 border ${accent ? "bg-gradient-to-br from-[#c8956c] to-[#b07d52] border-transparent text-white" : "bg-white border-gray-200"}`}>
      <p className={`text-xs font-medium ${accent ? "text-white/80" : "text-gray-500"}`}>{label}</p>
      <p className={`text-xl font-bold font-serif mt-1 ${accent ? "text-white" : "text-[#1e3a5f]"}`}>{value}</p>
    ...
    <div className="space-y-4">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h4 className="font-bold text-[#1e3a5f] text-sm mb-4">Rating Distribution</h4>
          <div className="h-48">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h4 className="font-bold text-[#1e3a5f] text-sm mb-4">Monthly Submissions</h4>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h4 className="font-bold text-[#1e3a5f] text-sm mb-4">Testimonials by Property</h4>
            ...
            <div className="space-y-2">
              {charts.byProperty.map((p) => (
                <div key={p.name} className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-[#1e3a5f] bg-[#1e3a5f]/5 rounded-full px-2.5 py-0.5">{p.count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h4 className="font-bold text-[#1e3a5f] text-sm mb-4">Testimonials by Investment Plan</h4>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex rounded-full border border-gray-200 overflow-hidden bg-white">
          <button onClick={() => setView("list")} className={`flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 transition ${view === "list" ? "bg-[#1e3a5f] text-white" : "text-[#1e3a5f]"}`}>
            <List className="w-3.5 h-3.5" /> Testimonials
          </button>
          <button onClick={() => setView("analytics")} className={`flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 transition ${view === "analytics" ? "bg-[#1e3a5f] text-white" : "text-[#1e3a5f]"}`}>
            <LayoutDashboard className="w-3.5 h-3.5" /> Analytics
          </button>
        </div>
        <Button onClick={() => { closeForm(); setFormOpen(true); }} className="ml-auto bg-[#1e3a5f] hover:bg-[#2d5a87]">
                ...
                {rows.map((t) => {
                  ...
                  return (
                    <div key={t.id} className="px-4 py-4 flex gap-3">
                      {t.photo ? (
                        <img src={t.photo} alt={t.customerName} className="w-11 h-11 rounded-full object-cover border-2 border-[#c8956c] shrink-0" />
                      ) : (
                        <div className="w-11 h-11 rounded-full bg-[#1e3a5f] text-white flex items-center justify-center font-bold shrink-0">
                          {t.customerName.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-bold text-[#1e3a5f] text-sm">{t.customerName}</p>
                          {t.investorId && (
                            <span title="Verified customer"><BadgeCheck className="w-4 h-4 text-[#c8956c]" /></span>
                          )}
                          <Stars value={t.rating} size="w-3.5 h-3.5" />
                          <StatusBadge status={t.status} />
                          {t.featured === "yes" && t.status === "approved" && (
                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#c8956c] text-white">Homepage ★</span>
                          )}
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <div className="flex gap-1">
                          ...
                          {t.status === "approved" && (
                            <>
                              <button
                                ...
                                className={`w-8 h-8 rounded-lg flex items-center justify-center transition ${t.featured === "yes" ? "bg-[#c8956c] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                              >
                          ...
                          <button title="Edit" onClick={() => openEdit(t)} className="w-8 h-8 rounded-lg bg-[#1e3a5f]/5 text-[#1e3a5f] flex items-center justify-center hover:bg-[#1e3a5f]/10">
      ...
      {formOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6">
            ...
            <h3 className="font-serif text-xl font-bold text-[#1e3a5f] mb-4">{editTarget ? "Edit Testimonial" : "Add Testimonial"}</h3>
                ...
                <div>
                  ...
                  {photo ? (
                    <div className="flex items-center gap-3">
                      <img src={photo.dataUrl} alt="Profile" className="w-10 h-10 rounded-full object-cover border-2 border-[#c8956c]" />
                      <button onClick={() => setPhoto(null)} className="text-xs text-red-500 hover:underline">Remove</button>
                    </div>
                  ) : (
                    <label className="inline-flex items-center gap-2 text-xs font-medium text-[#1e3a5f] border border-dashed border-gray-300 rounded-lg px-3 py-2 cursor-pointer hover:border-[#c8956c] transition">
      ...
      {reviewTarget && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-lg w-full shadow-2xl p-6">
            ...
            <h3 className="font-serif text-lg font-bold text-[#1e3a5f] mb-1">Review Testimonial</h3>
            <p className="text-xs text-gray-400 mb-3">{reviewTarget.customerName} · {reviewTarget.rating}★</p>
            <div className="bg-[#faf8f5] border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 max-h-40 overflow-y-auto mb-3">
              {reviewTarget.title && <p className="font-semibold text-[#1e3a5f] mb-1">{reviewTarget.title}</p>}

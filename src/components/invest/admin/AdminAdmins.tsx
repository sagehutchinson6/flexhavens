        ...
        <label
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm cursor-pointer transition ${
            value.includes(p.key)
              ? "border-[#c8956c] bg-[#c8956c]/10 text-[#1e3a5f] font-semibold"
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="text-lg font-bold text-[#1e3a5f] font-serif">Administrators</h3>
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]" onClick={() => setShowCreate(true)}>
        ...
        {adminsQuery.isLoading ? (
          ...
          <div className="space-y-3">
            {(adminsQuery.data ?? []).map((a) => (
              <div key={a.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white font-bold ${
                      a.role === "primary" ? "bg-[#c8956c]" : "bg-[#1e3a5f]"
                    }`}>
                    ...
                    <div>
                      <p className="font-bold text-[#1e3a5f] flex items-center gap-2">
                        ...
                        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                          a.role === "primary" ? "bg-[#c8956c]/15 text-[#b07d52]" : "bg-[#1e3a5f]/10 text-[#1e3a5f]"
                        }`}>
                          {a.role}
                        </span>
                        {a.status === "suspended" && (
      ...
      {showActivity && (
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          <h3 className="text-lg font-bold text-[#1e3a5f] font-serif mb-4">Administrator Activity Log</h3>
              ...
              <tbody className="divide-y">
                {(activityQuery.data ?? []).map((log) => (
                  <tr key={log.id} className="hover:bg-[#faf8f5]">
                    <td className="py-3 pr-4 font-semibold text-[#1e3a5f]">{log.adminName}</td>
                    <td className="py-3 pr-4">
                      <span className="bg-[#1e3a5f]/5 text-[#1e3a5f] text-xs font-semibold px-2 py-1 rounded-full">
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <UserPlus className="w-5 h-5 text-[#c8956c]" /> Add Administrator
              </h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input value={cName} onChange={(e) => setCName(e.target.value)} className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" placeholder="e.g. Jane Smith" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Email Address</label>
                <input type="email" value={cEmail} onChange={(e) => setCEmail(e.target.value)} className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" placeholder="jane@flexhavens.com" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Temporary Password</label>
                <input type="text" value={cPassword} onChange={(e) => setCPassword(e.target.value)} className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" placeholder="Min 10 chars, letters + numbers" />
              ...
              <Button
                className="w-full bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]"
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <Shield className="w-5 h-5 text-[#c8956c]" /> Edit Administrator
              </h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Display Name</label>
                <input value={eName} onChange={(e) => setEName(e.target.value)} className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" />
              </div>
              <div>
                  ...
                  {(["primary", "secondary"] as const).map((r) => (
                    <button
                      ...
                      className={`flex-1 px-4 py-2.5 rounded-lg border text-sm font-semibold capitalize transition ${
                        eRole === r ? "border-[#1e3a5f] bg-[#1e3a5f] text-white" : "border-gray-200 text-gray-600"
                      }`}
                    >
                      {r}
                    </button>

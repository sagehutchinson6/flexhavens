...
const inputCls =
  "mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]";
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="text-lg font-bold text-[#1e3a5f] font-serif">Mortgage Plans</h3>
            <p className="text-sm text-gray-500 mt-0.5">Create plans, then assign them to properties under Property Settings.</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => setView("plans")}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${view === "plans" ? "bg-[#1e3a5f] text-white" : "bg-[#faf8f5] text-gray-600"}`}
            >
              Plans
            </button>
            <button
              onClick={() => setView("properties")}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${view === "properties" ? "bg-[#1e3a5f] text-white" : "bg-[#faf8f5] text-gray-600"}`}
            >
              Property Settings
            </button>
            <Button size="sm" className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]" onClick={openCreate}>
        ...
        {view === "plans" ? (
          plansQuery.isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="font-semibold text-[#1e3a5f]">No mortgage plans yet</p>
              <p className="text-sm text-gray-500 mt-1">Create your first monthly or yearly mortgage plan.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
              {plans.map((p) => (
                <div key={p.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center mb-4">
                    <div className="bg-white rounded-lg py-2">
                      ...
                      <p className="text-sm font-bold text-[#b07d52]">{p.downPaymentPercent}%</p>
                    </div>
                    <div className="bg-white rounded-lg py-2">
                      <p className="text-[10px] text-gray-400 uppercase">Interest</p>
                      <p className="text-sm font-bold text-[#1e3a5f]">{p.interestPercent}%</p>
                    </div>
                    <div className="bg-white rounded-lg py-2">
          ...
          <div className="space-y-3">
            {(productsQuery.data ?? []).map((p) => (
              <div key={p.id} className="bg-[#faf8f5] rounded-xl p-5 border border-gray-100 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <Landmark className="w-5 h-5 text-[#c8956c]" /> {editId ? "Edit Mortgage Plan" : "New Mortgage Plan"}
              </h3>
      ...
      {editProduct && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setEditProduct(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <Home className="w-5 h-5 text-[#c8956c]" /> {editProduct.name}
              </h3>
              <button onClick={() => setEditProduct(null)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <label className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition ${pEnabled ? "border-[#c8956c] bg-[#c8956c]/5" : "border-gray-200"}`}>
                <input type="checkbox" checked={pEnabled} onChange={(e) => setPEnabled(e.target.checked)} className="accent-[#1e3a5f] w-4 h-4" />
                <span>
                  <span className="block text-sm font-bold text-[#1e3a5f]">Enable Mortgage Purchase</span>
                    ...
                    {activePlans.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {activePlans.map((plan) => (
                          <label key={plan.id} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg border text-sm cursor-pointer transition ${pPlanIds.includes(plan.id) ? "border-[#1e3a5f] bg-[#1e3a5f]/5 font-semibold text-[#1e3a5f]" : "border-gray-200 text-gray-600"}`}>
                            <input
                              type="checkbox"

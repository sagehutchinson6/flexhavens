  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center gap-4">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xl font-bold text-[#1e3a5f] font-serif truncate">{value}</p>
      </div>
    </div>
  );
}
...
export default function AdminMortgages() {
  ...
  const exportRows = () => ({
    columns: ["Reference", "Investor", "Email", "Property", "Plan", "Price", "Total Payable", "Paid", "Remaining", "Installment", "Frequency", "Status", "Applied", "Next Payment"],
    rows: rows.map((m: any) => [
  ...
  const exportCsv = () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `flexhavens-mortgages-${statusFilter}-${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.text("FlexHavens Mortgage Report", 14, 18);
      doc.setFontSize(10);
      ...
      doc.save(`flexhavens-mortgages-${statusFilter}-${new Date().toISOString().slice(0, 10)}.pdf`);
  ...
  const reviewActions = useMemo(() => {
    ...
    if (detail.status === "pending") {
      acts.push({ action: "approve", label: "Approve", icon: CheckCircle2, cls: "bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87] text-white" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatCard icon={Landmark} label="Total Mortgages" value={String(stats?.total ?? 0)} tone="bg-[#1e3a5f]/5 text-[#1e3a5f]" />
        ...
        <StatCard icon={Banknote} label="Revenue Collected" value={formatCurrency(stats?.revenue ?? 0)} tone="bg-[#c8956c]/10 text-[#b07d52]" />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="flex gap-2">
            <button onClick={() => setView("mortgages")} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${view === "mortgages" ? "bg-[#1e3a5f] text-white" : "bg-[#faf8f5] text-gray-600"}`}>
              Mortgages
            </button>
            <button onClick={() => setView("reports")} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${view === "reports" ? "bg-[#1e3a5f] text-white" : "bg-[#faf8f5] text-gray-600"}`}>
              Reports & Analytics
            </button>
          </div>
            ...
            <>
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm w-full xl:w-64 focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]"
                />
              </div>
          ...
          <>
              ...
              {statusFilters.map((f) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold capitalize transition ${
                    statusFilter === f ? "bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87] text-white shadow" : "bg-[#faf8f5] text-gray-600 hover:text-[#1e3a5f]"
                  }`}
                >
                  {f}
            ...
            {listQuery.isLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="font-semibold text-[#1e3a5f]">No mortgages found</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {rows.map((m: any) => (
                      <tr key={m.id} className="hover:bg-[#faf8f5] transition">
                        ...
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              <p className="font-semibold text-[#1e3a5f] truncate">{m.investorName}</p>
                              <p className="text-xs text-gray-400 truncate">{m.investorEmail}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 pr-4">
                          <p className="font-medium text-[#1e3a5f]">{m.propertyName}</p>
                        ...
                        <td className="py-4 pr-4 font-bold text-[#1e3a5f]">{formatCurrency(m.remainingBalance)}</td>
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2">
                            <ProgressBar value={m.progress} className="w-16" />
                            <span className="text-xs font-semibold text-[#1e3a5f]">{m.progress}%</span>
                          </div>
                        </td>
          ...
          reportQuery.isLoading ? (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard icon={Home} label="Outright Purchases" value={String(report.totalOutright)} tone="bg-[#1e3a5f]/5 text-[#1e3a5f]" />
                <StatCard icon={Landmark} label="Mortgage Purchases" value={String(report.totalMortgagePurchases)} tone="bg-[#c8956c]/10 text-[#b07d52]" />
              ...
              <div>
                <h4 className="font-bold text-[#1e3a5f] mb-3 flex items-center gap-2">
                  <CalendarClock className="w-4 h-4 text-[#c8956c]" /> Collections — Last 12 Months
                </h4>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                    ...
                    <tbody className="divide-y">
                      {report.series.map((r) => (
                        <tr key={r.month} className="hover:bg-[#faf8f5]">
                          <td className="py-2.5 pr-4 text-gray-600">{r.month}</td>
                          <td className="py-2.5 font-semibold text-[#1e3a5f]">{formatCurrency(r.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-white rounded-t-2xl z-10">
              <h3 className="text-lg font-bold text-[#1e3a5f] font-serif flex items-center gap-2">
                <Landmark className="w-5 h-5 text-[#c8956c]" /> {selected.reference}
              </h3>
            ...
            {detailQuery.isLoading || !detail ? (
              ...
              <div className="p-6 space-y-5">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="bg-[#faf8f5] rounded-xl p-4">
                    ...
                    <div className="flex items-center gap-3 mb-1.5">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-[#1e3a5f] truncate">{detail.investor?.name}</p>
                  ...
                  <div className="bg-[#faf8f5] rounded-xl p-4">
                    ...
                    <p className="font-bold text-[#1e3a5f]">{detail.propertyName}</p>
                ...
                <div className="border border-gray-200 rounded-xl p-4 space-y-2.5 text-sm">
                  <div className="flex justify-between"><span className="text-gray-500">Property Price</span><span className="font-semibold text-[#1e3a5f]">{formatCurrency(detail.propertyPrice)}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Down Payment</span><span className="font-semibold text-[#b07d52]">{formatCurrency(detail.downPaymentAmount)}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Total Contract Value</span><span className="font-bold text-[#1e3a5f]">{formatCurrency(detail.totalPayable)}</span></div>
                  ...
                  <div className="flex justify-between border-t pt-2.5"><span className="font-bold text-[#1e3a5f]">Remaining Balance</span><span className="text-lg font-bold text-[#c8956c]">{formatCurrency(detail.remainingBalance)}</span></div>
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    <span className="text-xs font-bold text-[#1e3a5f]">{Math.min(Math.round((Number(detail.amountPaid) / (Number(detail.totalPayable) || 1)) * 100), 100)}%</span>
                  </div>
                </div>
                ...
                {reviewActions.length > 0 && (
                  <div className="space-y-3">
                    <div>
                      ...
                      <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} className="mt-1.5 w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c] resize-none" placeholder="Optional note (shared with the investor)..." />
                    </div>
                ...
                {["approved", "active", "suspended"].includes(detail.status) && (
                  <div className="bg-[#faf8f5] rounded-xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <input type="number" min="1" step="0.01" value={adjAmount} onChange={(e) => setAdjAmount(e.target.value)} placeholder="Amount (₦)" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" />
                      <input value={adjNote} onChange={(e) => setAdjNote(e.target.value)} placeholder="Reason (required)" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]" />
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                ...
                <div>
                  ...
                  {detail.payments.length === 0 ? (
                    ...
                    <div className="overflow-x-auto border border-gray-100 rounded-xl">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-left text-xs uppercase tracking-wider text-gray-400 border-b bg-[#faf8f5]">
                        ...
                        <tbody className="divide-y">
                          {detail.payments.map((p: any) => (
                            <tr key={p.id}>
                              ...
                              <td className="py-2.5 px-3 font-semibold text-[#c8956c]">{formatCurrency(p.amount)}</td>
                              <td className="py-2.5 px-3 font-semibold text-[#1e3a5f]">{formatCurrency(p.remainingBalanceAfter)}</td>
                              <td className="py-2.5 px-3 text-gray-500 capitalize">{p.method === "manual_adjustment" ? "Adjustment" : "Wallet"}</td>
                              <td className="py-2.5 px-3 font-mono text-xs text-[#b07d52]">{p.receiptNo}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>

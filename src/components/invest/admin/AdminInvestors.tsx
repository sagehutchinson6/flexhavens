...
export default function AdminInvestors() {
  ...
  const setStatus = trpc.investAdmin.setInvestorStatus.useMutation({
    onSuccess: () => { toast.success("Investor status updated"); refetch(); },
  ...
  return (
    ...
    >
      {investors && investors.length > 0 ? (
                ...
                <tr key={inv.id} className="hover:bg-[#faf8f5] transition">
                  <td className="py-3.5 pr-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-[#1e3a5f] flex items-center justify-center text-white font-bold text-xs shrink-0 overflow-hidden">
                        {inv.avatar ? (
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-[#1e3a5f] truncate">
                          {inv.name}
                          {inv.role === "admin" && (
                            <span className="ml-2 text-[10px] font-bold uppercase bg-[#c8956c]/10 text-[#b07d52] px-2 py-0.5 rounded-full">Admin</span>
                          )}
                        </p>
                  ...
                  <td className="py-3.5 pr-4 font-bold text-[#1e3a5f]">
                  ...
                  <td className="py-3.5">
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={() => setDetailId(inv.id)} className="border-[#1e3a5f] text-[#1e3a5f] h-8 text-xs">
        ...
        <EmptyState icon={Users} title="No investors found" text="Registered investors will appear here." />
      )}
              ...
              <>
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-[#1e3a5f] font-serif">{detail.investor.name}</h3>
                ...
                <div className="flex items-center justify-between bg-blue-50/60 border border-blue-100 rounded-xl px-4 py-3 mb-5">
                  <p className="text-sm text-blue-900 font-medium flex items-center gap-2">
                    ...
                    {detail.financials?.walletFrozen
                      ? "Wallet is frozen — withdrawals & investments blocked"
                      : "Wallet is active — freeze to block withdrawals & investments"}
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                  {[
                    { label: "Wallet Balance", value: formatCurrency(detail.financials?.walletBalance ?? 0), cls: "text-[#1e3a5f]" },
                    { label: "Available Balance", value: formatCurrency(detail.financials?.availableBalance ?? 0), cls: "text-[#1e3a5f]" },
                    { label: "Active Investments", value: formatCurrency(detail.financials?.activeInvestments ?? 0), cls: "text-[#1e3a5f]" },
                    ...
                    { label: "Withdrawal Count", value: String(detail.financials?.withdrawalCount ?? 0), cls: "text-[#1e3a5f]" },
                    { label: "Monthly ROI Paid", value: formatCurrency(detail.financials?.monthlyRoiPaid ?? 0), cls: "text-[#c8956c]" },
                    ...
                    <div key={c.label} className="bg-[#faf8f5] rounded-xl p-4">
                ...
                <h4 className="font-bold text-[#1e3a5f] mb-3">Investment History ({detail.investments.length})</h4>
                <div className="space-y-2 mb-6 max-h-52 overflow-y-auto">
                  {detail.investments.length > 0 ? detail.investments.map((inv: any) => (
                    <div key={inv.id} className="bg-[#faf8f5] rounded-lg p-3.5">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-semibold text-[#1e3a5f]">#{inv.id} · {inv.projectName}</p>
                ...
                <h4 className="font-bold text-[#1e3a5f] mb-3 flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-[#c8956c]" /> Recent Activity ({detail.activity.length})
                </h4>
                <div className="space-y-2 max-h-52 overflow-y-auto">
                  {detail.activity.length > 0 ? detail.activity.map((a: any) => (
                    <div key={a.id} className="flex items-start justify-between gap-3 text-sm border-b border-gray-100 pb-2">
                      <div>
                        <p className="font-medium text-[#1e3a5f] capitalize">{a.action.replace(/_/g, " ")}</p>

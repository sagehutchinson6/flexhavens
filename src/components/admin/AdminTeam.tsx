  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-[#1e3a5f] font-serif">Team Section</h2>
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={openAdd} className="bg-[#1e3a5f]">
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-3">
          {list.map((m, i) => (
            <div key={m.id} className="flex items-center gap-4 border border-gray-200 rounded-2xl p-4">
              <img
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[#b07d52] font-medium mt-0.5">{m.role}</p>
                {m.bio && <p className="text-xs text-gray-400 mt-1 line-clamp-2">{m.bio}</p>}
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button title="Move up" disabled={i === 0 || reorder.isPending} onClick={() => move(i, -1)} className="p-2 rounded-lg text-gray-400 hover:text-[#1e3a5f] hover:bg-gray-100 transition disabled:opacity-30">
                  <ChevronUp className="w-4 h-4" />
                </button>
                <button title="Move down" disabled={i === list.length - 1 || reorder.isPending} onClick={() => move(i, 1)} className="p-2 rounded-lg text-gray-400 hover:text-[#1e3a5f] hover:bg-gray-100 transition disabled:opacity-30">
                  <ChevronDown className="w-4 h-4" />
                </button>
                <button title={m.isActive === "yes" ? "Hide from website" : "Show on website"} onClick={() => toggleActive(m)} className="p-2 rounded-lg text-gray-400 hover:text-[#1e3a5f] hover:bg-gray-100 transition">
                  {m.isActive === "yes" ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
                <button title="Edit" onClick={() => openEdit(m)} className="p-2 rounded-lg text-gray-400 hover:text-[#1e3a5f] hover:bg-gray-100 transition">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 mt-2">
            <div className="flex items-center gap-4">
              ...
              />
              <div>
                <label className={`inline-flex items-center gap-2 px-4 h-10 rounded-lg border border-gray-300 text-sm font-semibold text-[#1e3a5f] cursor-pointer hover:border-[#1e3a5f] transition ${photoBusy ? "opacity-50 pointer-events-none" : ""}`}>
            ...
            <Button onClick={handleSave} disabled={save.isPending || photoBusy} className="w-full bg-[#1e3a5f]">

...
function SocialLinks({ bgColor }: { bgColor: string }) {
  return (
    ...
    >
      ...
      <a
        href="https://tiktok.com/@flexhavens"
        target="_blank"
        rel="noopener noreferrer"
        className="hover:scale-110 transition-transform"
        title="TikTok"
      ...
      <a
        href="https://facebook.com/flexhavens"
        target="_blank"
        rel="noopener noreferrer"
        className="hover:scale-110 transition-transform"
        title="Facebook"
        ...
        <div className="px-4 w-full">
          <div key={rotateIndex} className="animate-[fadein_0.5s_ease-in] inline-flex">
            <span className="truncate inline-flex max-w-full">
              <MessageSpan m={messages[rotateIndex % messages.length]} iconColor="#c8956c" />
            </span>
          </div>
        </div>
      ) : (
        <div className={`animate-scroll-x inline-flex items-center gap-8 px-4 ${pauseClass}`} style={marqueeStyle}>
          {[0, 1, 2].map((copy) => (
            <span key={copy} className="inline-flex items-center gap-8">
              {messages.map((m) => (
                <MessageSpan key={`${copy}-${m.id}`} m={m} iconColor="#c8956c" />
              ))}
            </span>
          ))}
        </div>
...
export default function AnnouncementBar() {
  ...
  return <AnnouncementTicker messages={data.announcements} settings={data.settings} />;
}

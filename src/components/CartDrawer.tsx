  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <h2 className="text-lg font-bold text-[#1e3a5f]">Your Cart ({totalItems})</h2>
        {onClose && (
          <button
            onClick={handleClose}
      ...
      <div className="flex-1 overflow-y-auto py-4 px-4 space-y-4">
        {items.map((item) => (
          <div key={item.productId} className="flex gap-4 bg-gray-50 p-4 rounded-xl">
            <img
              src={item.image}
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-[#1e3a5f] truncate">{item.productName}</h4>
              <p className="text-sm text-gray-500">{item.size} | {item.bedrooms}BR/{item.bathrooms}BA</p>
              <p className="font-bold text-[#1e3a5f] mt-1">
      ...
      <div className="space-y-4 px-4 pb-6">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xl font-bold text-[#1e3a5f]">₦{totalPrice.toLocaleString()}</span>
        </div>

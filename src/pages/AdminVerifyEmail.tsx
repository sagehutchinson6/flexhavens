  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 text-center">
        <div className="w-14 h-14 bg-[#1e3a5f] rounded-xl flex items-center justify-center mx-auto mb-6">
        ...
        {status === "success" && (
          <>
            ...
            <h3 className="text-lg font-bold text-[#1e3a5f] mb-2">Email Address Updated</h3>
            <p className="text-sm text-gray-600 mb-6">
              Your new administrator email is verified and active. Use it the next time you sign in.
            </p>
            <Link to="/admin">
              <Button className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87]">Go to Admin Login</Button>
            </Link>
          </>
        )}
        ...
        {(status === "error" || status === "no-token") && (
          <>
            ...
            <Link to="/admin">
              <Button variant="outline" className="border-[#1e3a5f] text-[#1e3a5f]">Back to Admin Login</Button>
            </Link>
          </>
        )}
      </div>

...
import { CountrySelect, NigerianStateSelect } from "@/components/GeoSelects";
        ...
        <main className="pt-24 pb-20">
          <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-white rounded-2xl p-8 sm:p-12 shadow-sm text-center">
              <div className="w-16 h-16 bg-[#1e3a5f] rounded-full flex items-center justify-center mx-auto mb-5">
                <Landmark className="w-8 h-8 text-[#c8956c]" />
              </div>
              <h1 className="text-3xl font-serif font-bold text-[#1e3a5f] mb-3">Application Submitted</h1>
              <p className="text-gray-500 mb-2">
                Your mortgage application is now <span className="font-semibold text-amber-600">pending admin approval</span>.
              </p>
              <p className="text-sm text-gray-400 mb-6">
                Reference: <span className="font-mono font-semibold text-[#1e3a5f]">{mortgageDone.reference}</span>
              </p>
              <div className="bg-[#faf8f5] rounded-xl p-4 text-sm text-gray-500 mb-8">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <Link to="/#catalog">
                  <Button variant="outline" className="border-[#1e3a5f] text-[#1e3a5f]">Back to Catalog</Button>
                </Link>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-[#faf8f5]">
      ...
      <main className="pt-24 pb-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-4xl font-serif font-bold text-[#1e3a5f] mb-8">Checkout</h1>

          {items.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
              <ShoppingBag className="w-16 h-16 text-gray-300 mx-auto mb-4" />
              <h2 className="text-xl font-bold text-[#1e3a5f] mb-2">Your cart is empty</h2>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                {mortgageAvailable && singleItem && (
                  <div className="bg-white rounded-2xl p-6 shadow-sm">
                    <h2 className="text-xl font-bold text-[#1e3a5f] mb-6">Purchase Method</h2>
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      >
                        <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${
                          purchaseMethod === "outright" ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-500"
                        }`}>
                          <ShoppingBag className="w-5 h-5" />
                        </div>
                        <p className="font-bold text-[#1e3a5f]">Buy Outright</p>
                      ...
                      >
                        <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${
                          purchaseMethod === "mortgage" ? "bg-[#c8956c] text-white" : "bg-gray-100 text-gray-500"
                        }`}>
                          <Landmark className="w-5 h-5" />
                        </div>
                        <p className="font-bold text-[#1e3a5f]">Buy with Mortgage</p>
                        <p className="text-sm text-gray-500">
                    ...
                    {purchaseMethod === "mortgage" && (
                      <div className="mt-6 space-y-4">
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Select a Mortgage Plan</p>
                          ...
                          {mortgagePlans.map((p) => (
                            ...
                            >
                              <div className="flex-1 min-w-[140px]">
                                <p className="font-bold text-[#1e3a5f]">{p.name}</p>
                              ...
                              <div className="text-sm">
                                ...
                                <p className="font-bold text-[#b07d52]">{formatCurrency(p.downPayment)}</p>
                              </div>
                              <div className="text-sm">
                                <p className="text-[11px] text-gray-400">{p.paymentFrequency === "yearly" ? "Yearly" : "Monthly"} installment</p>
                                <p className="font-bold text-[#c8956c]">{formatCurrency(p.installment)}</p>
                              </div>
                              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                                selectedPlanId === p.id ? "border-[#1e3a5f]" : "border-gray-300"
                              }`}>
                                {selectedPlanId === p.id && <div className="w-2.5 h-2.5 rounded-full bg-[#1e3a5f]" />}
                              </div>
                            </button>
                          ))}
                        </div>
                        ...
                        {selectedPlan && (
                          <div className="bg-[#faf8f5] rounded-xl p-5 space-y-2.5 text-sm">
                            <div className="flex justify-between"><span className="text-gray-500">Property price</span><span className="font-semibold text-[#1e3a5f]">{formatCurrency(singleItem.price)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">Down payment (due to activate)</span><span className="font-bold text-[#b07d52]">{formatCurrency(selectedPlan.downPayment)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">{selectedPlan.paymentFrequency === "yearly" ? "Yearly" : "Monthly"} repayment</span><span className="font-bold text-[#c8956c]">{formatCurrency(selectedPlan.installment)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">Duration</span><span className="font-semibold text-[#1e3a5f]">{selectedPlan.durationMonths} months ({selectedPlan.periods} payments)</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">Next payment date</span><span className="font-semibold text-[#1e3a5f]">{formatDate(addPeriodClient(new Date(), selectedPlan.paymentFrequency))}</span></div>
                            <div className="flex justify-between border-t pt-2.5"><span className="font-semibold text-[#1e3a5f]">Total payable</span><span className="font-bold text-[#1e3a5f] text-base">{formatCurrency(selectedPlan.totalPayable)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">Estimated completion</span><span className="font-semibold text-[#1e3a5f]">{formatDate(estimatedCompletionClient(new Date(), selectedPlan.durationMonths))}</span></div>
                          </div>
                        )}
                ...
                {purchaseMethod === "mortgage" ? (
                  <div className="bg-white rounded-2xl p-6 shadow-sm">
                    <h2 className="text-xl font-bold text-[#1e3a5f] mb-4">Applicant Information</h2>
                    {isAuthenticated && investor ? (
                      <div className="flex items-center gap-4 bg-[#faf8f5] rounded-xl p-4">
                        <div className="w-11 h-11 bg-[#1e3a5f] rounded-xl flex items-center justify-center shrink-0">
                          <User className="w-5 h-5 text-[#c8956c]" />
                        </div>
                        <div>
                          <p className="font-semibold text-[#1e3a5f]">{investor.name}</p>
                          <p className="text-xs text-gray-400">{investor.email}</p>
                          <p className="text-xs text-gray-400 mt-0.5">The application is filed against your investor account.</p>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-amber-800">Investor account required</p>
                  ...
                  <div className="bg-white rounded-2xl p-6 shadow-sm">
                    <h2 className="text-xl font-bold text-[#1e3a5f] mb-6">Buyer Information</h2>
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                      ...
                      <div>
                        ...
                        <CountrySelect
                          ...
                          className="mt-1.5 w-full h-10 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#c8956c]/40 focus:border-[#c8956c]"
                        />
                      </div>
                      <div>
                ...
                {purchaseMethod === "outright" && (
                  <div className="bg-white rounded-2xl p-6 shadow-sm">
                    <h2 className="text-xl font-bold text-[#1e3a5f] mb-6">Payment Method</h2>
                    <div className="space-y-3">
                      {paymentMethods.map((method) => (
                        ...
                        >
                          <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${
                            paymentMethod === method.id ? "bg-[#1e3a5f] text-white" : "bg-gray-100 text-gray-500"
                          }`}>
                            <method.icon className="w-5 h-5" />
                          </div>
                          <div className="flex-1">
                            <p className="font-semibold text-[#1e3a5f]">{method.label}</p>
                            <p className="text-sm text-gray-500">{method.note}</p>
                          </div>
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                            paymentMethod === method.id ? "border-[#1e3a5f]" : "border-gray-300"
                          }`}>
                            {paymentMethod === method.id && (
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm sticky top-24">
                    ...
                    {items.map((item) => (
                      <div key={item.productId} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-[#1e3a5f] text-sm truncate">{item.productName}</p>
                  ...
                  {purchaseMethod === "mortgage" && selectedPlan ? (
                    <div className="border-t mt-6 pt-4 space-y-2">
                      <p className="text-xs font-semibold text-[#c8956c] uppercase tracking-wider flex items-center gap-1.5 mb-1">
                        <Landmark className="w-3.5 h-3.5" /> Mortgage — {selectedPlan.name}
                      </p>
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Total contract value</span>
                        <span className="font-semibold text-[#1e3a5f]">{formatCurrency(selectedPlan.totalPayable)}</span>
                      </div>
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Down payment</span>
                        <span className="font-semibold text-[#b07d52]">{formatCurrency(selectedPlan.downPayment)}</span>
                      </div>
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>{selectedPlan.paymentFrequency === "yearly" ? "Yearly" : "Monthly"} installment</span>
                        <span className="font-semibold text-[#c8956c]">{formatCurrency(selectedPlan.installment)}</span>
                      </div>
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Duration</span>
                    ...
                    <div className="border-t mt-6 pt-4 space-y-2">
                      ...
                      <div className="flex justify-between text-lg font-bold text-[#1e3a5f] pt-2 border-t">
                        <span>Total</span>
                        <span>₦{totalPrice.toLocaleString()}</span>
                      </div>
                  ...
                  <Button
                    ...
                    className="w-full mt-6 h-12 bg-gradient-to-r from-[#1e3a5f] to-[#2d5a87] hover:shadow-lg transition text-base font-semibold"
                  >
                    {purchaseMethod === "mortgage" ? (
                      <>

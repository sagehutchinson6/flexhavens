...
const investTabs: { id: SectionId; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: "liquidations", label: "Liquidations", icon: CircleDollarSign },
  { id: "mortgagePlans", label: "Mortgage Plans", icon: Landmark },
  { id: "mortgages", label: "Mortgages", icon: Banknote },
  ...
  { id: "profits", label: "Profits / ROI", icon: Coins },
...
export default function UnifiedAdminDashboard() {
    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 border-b border-white/10">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-serif text-lg font-bold text-white">FlexHavens</span>
        </Link>
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex">
      {/* Desktop sidebar */}
      <aside className="hidden lg:block w-64 shrink-0 bg-[#1e3a5f] fixed inset-y-0 left-0 z-30">
        {sidebarContent}
      </aside>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          />
          <aside className="absolute inset-y-0 left-0 w-64 bg-[#1e3a5f] z-50">
            <button
              onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-white border-b border-gray-200 px-4 sm:px-6 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-serif text-xl font-bold text-[#1e3a5f] truncate">
              {currentLabel}
            </h1>
          ...
          {me && (
            <button
              ...
              title="My Profile"
            >
              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold overflow-hidden ${me.role === "primary" ? "bg-[#c8956c]" : "bg-[#1e3a5f]"}`}>
                {me.avatar ? <img src={me.avatar} alt="" className="w-full h-full object-cover" /> : me.displayName.charAt(0).toUpperCase()}
              </span>
              <span className="text-left">
                <span className="block text-xs font-bold text-[#1e3a5f] leading-tight">{me.displayName}</span>
          ...
          <button
            ...
            className="relative p-2 rounded-full hover:bg-gray-100 text-[#1e3a5f] transition-colors"
          ...
          <button
            onClick={logout}
            className="hidden sm:flex items-center gap-2 text-sm text-gray-600 hover:text-[#1e3a5f] transition-colors"
          >
            <LogOut className="w-4 h-4" />
            Log out
          </button>
